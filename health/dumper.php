...
if (!empty($_POST['login']) && isset($_POST['pass'])) {
        if ($_POST['login'] == 'senia' && md5($_POST['pass']) == '3bf1114a986ba87ed28fc1b5884fc2f8') {
...
if (!$auth || $_SERVER['QUERY_STRING'] == 'reload') {
        setcookie("skd");
        print tpl_page(tpl_auth($error ? tpl_error($error) : ''), "<SCRIPT>if (jsEnabled) {document.write('<INPUT TYPE=submit VALUE=Применить>');}</SCRIPT>");
        print "<SCRIPT>document.getElementById('timer').innerHTML = '" . round(array_sum(explode(' ', microtime())) - $timer, 4) . " сек.'</SCRIPT>";
        exit;
}
...
class dumper {
        ...
        function main(){
                ...
                $buttons = "<INPUT TYPE=submit VALUE=Применить><INPUT TYPE=button VALUE=Выход onClick=\"location.href = '".MY_NAME."?reload'\">";
                echo tpl_page(tpl_main(), $buttons);
        }

        function db_select(){
                if (DB_DATABASE != '') {
                        $items = explode(',', trim(DB_DATABASE));
  ...
  function fn_read_str($fp){
    $nl_pos = strpos($this->file_cache, "\n");
    if ( $nl_pos===false ) {
      // need read more data
      //while (!$string && ($str = $this->fn_read($fp))){
      while ( $str = $this->fn_read($fp) ){
        $nl_pos = strpos($str, "\n");
        if ( $nl_pos===false ) {
          $this->file_cache .= $str;
        }else{
          $string = $this->file_cache . substr($str, 0, $nl_pos);
          if ( ($nl_pos+1)==strlen($str) ) {
            $this->file_cache = '';
          }else{
            $this->file_cache = substr($str, $nl_pos + 1);
            $this->file_cache = ltrim($this->file_cache);
          }
          return trim($string);
        }
      }
      if ( strlen($this->file_cache) ) {
      ...
      // have in cache - cut & ret
      $string = substr($this->file_cache, 0, $nl_pos);
      if ( ($nl_pos+1)==strlen($this->file_cache) ) {
        $this->file_cache='';
      }else{
        $this->file_cache = substr($this->file_cache, $nl_pos + 1);
      }
      $this->file_cache = ltrim($this->file_cache);
      return trim($string);
    }
  }
        function _fn_read_str($fp){ // old function 2008-04-29
                $string = '';
                $this->file_cache = ltrim($this->file_cache);
                ...
                if ($pos < 1) {
                        while (!$string && ($str = $this->fn_read($fp))){
                            $pos = strpos($str, "\n", 0);
// FIX 2008-02-28 - crash if readed data start from \n
