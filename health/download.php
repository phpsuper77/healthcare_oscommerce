...
// Check that order_id, customer_id and filename match
  ...
  if (!tep_db_num_rows($downloads_query)) {
    $messageStack->add_session('download', TEXT_DOWNLOAD_PRODUCT_NOT_FOUND);
...
// Die if time expired (maxdays = 0 means no time limit)
  ...
  {
    $messageStack->add_session('download', sprintf(TEXT_DOWNLOAD_PRODUCT_EXPIRED, tep_datetime_short($download_timestamp)));
    tep_redirect(tep_href_link(FILENAME_ACCOUNT_HISTORY_INFO, 'order_id=' . (int)$HTTP_GET_VARS['order'], 'SSL'));
  }
// Die if remaining count is <=0
  if ($downloads['download_count'] <= 0) 
  {
    $messageStack->add_session('download', sprintf(TEXT_DOWNLOAD_PRODUCT_EXPIRED_DOWNLOADS, $downloads['download_count_1']));
    tep_redirect(tep_href_link(FILENAME_ACCOUNT_HISTORY_INFO, 'order_id=' . (int)$HTTP_GET_VARS['order'], 'SSL'));
  }
    
// Die if file is not there
  if (!file_exists(DIR_FS_DOWNLOAD . $downloads['orders_products_filename'])) {
    $messageStack->add_session('download', TEXT_DOWNLOAD_FILE_NOT_FOUND);
    tep_redirect(tep_href_link(FILENAME_DEFAULT));
  }
  
// Now decrement counter
  tep_db_query("update " . TABLE_ORDERS_PRODUCTS_DOWNLOAD . " set download_count = download_count-1, download_count_1 = download_count_1+1 where orders_products_download_id = '" . (int)$HTTP_GET_VARS['id'] . "'");
