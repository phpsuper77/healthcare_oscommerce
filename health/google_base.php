...
class googlebase_feed extends feed_base_txt{
  ...
  function googlebase_feed(){
    ...
    $this->columns( array(
                      ...
                      'availability'  => array('value'=>'In stock'),
					  'google product category'  => array('value'=>'Health &amp; Beauty > Health Care'),
					  'product_type'  => array('value'=>'Health &amp; Beauty > Health Care')
                    ) );
    ...
    $pQuery = "SELECT DISTINCT p.products_id, if(length(p.products_model),p.products_model,'none') as products_model, ".
                "p.products_weight, if(length(m.manufacturers_name),m.manufacturers_name,'none') as manufacturers_name , ".
                ...
                "if(length(p.products_mpn),p.products_mpn,if(length(p.products_model),p.products_model,'none')) as products_mpn, p.products_ean, p.products_ean, ".
                "p.products_image, p.products_image_med, p.products_image_lrg, c.categories_id ".
              "FROM ".
  ...
  function before_out(){
    ...
    //_image_link_
    $image_file = '';
    $this->_data['product type'] = seo_categories_path($this->_data['categories_id']);
    ...
    if ( !empty($image_file) ) {
      $this->_data['_image_link_'] = HTTP_SERVER.DIR_WS_HTTP_CATALOG.DIR_WS_IMAGES.$image_file;
        ...
        $this->_data['_image_link_'] = 'none';
    }
     $this->_data['product type'] = get_category_all_names(tep_get_product_parent_category_id($this->_data['products_id']), '/');
     $this->_data['google product category'] = get_category_all_names(tep_get_product_parent_category_id($this->_data['products_id']), '>');
...
function get_category_all_names($cat_array,$separate) {
    $array = explode('_', $cat_array);
    $a = 0;
    $name = '';
    $count = count($array);
    while ($a < $count) {
        $name .= ( $a == 0 ? '' : $separate) . urldecode(tep_get_categories_name($array[$a]));
        $a++;
    }
    return $name;
}
