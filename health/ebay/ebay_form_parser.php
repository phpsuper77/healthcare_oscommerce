<?php
  ...
  class ebayForm_xmlParser {
    ...
    */
    function ebayForm_xmlParser($input, $xmlParams=array(XML_OPTION_CASE_FOLDING => 0)) {
      ...
      if(xml_parse_into_struct($xmlp, $input, $vals, $index)) {
        ...
        $error_code = xml_get_error_code($xmlp);
        echo '<pre>'; var_dump($error_code, 
                               xml_error_string($error_code), 
                               'Line :'.xml_get_current_line_number($xmlp), 
                               'Col :'.xml_get_current_column_number($xmlp) ); echo '</pre>';
      }
