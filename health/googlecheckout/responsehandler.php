...
if ($file = @fopen($curr_dir."/googlecheckout/logs/request.xml", "w")){
  @fwrite($file, $xml_response);
  @fclose($file);
}

//$xml_response = file_get_contents($curr_dir."/googlecheckout/logs/request.xml");

//echo '<pre>'; var_dump($xml_response); echo '</pre>'; die;
...
//       Order already processed, send ACK http 200 to avoid notification resend
      ...
      if(isset($gc_data[$root]['shopping-cart']['merchant-private-data']['search_engines_id']['VALUE'])) {
        $search_engines_id = $gc_data[$root]['shopping-cart']['merchant-private-data']['search_engines_id']['VALUE'];
        $search_words_id = $gc_data[$root]['shopping-cart']['merchant-private-data']['search_words_id']['VALUE'];
      }
      if(!empty($gc_data[$root]['shopping-cart']['merchant-private-data']['cc_id']['VALUE'])) {
        $cc_id = $gc_data[$root]['shopping-cart']['merchant-private-data']['cc_id']['VALUE'];
        $coupon_type = $gc_data[$root]['shopping-cart']['merchant-private-data']['coupon_type']['VALUE'];
      }
      if(isset($gc_data[$root]['shopping-cart']['merchant-private-data']['customer_groups_id']['VALUE'])) {
        $customer_groups_id = $gc_data[$root]['shopping-cart']['merchant-private-data']['customer_groups_id']['VALUE'];
      }
...
//    GC email is the most important one
      if ($customer_exists['customers_id'] != '') {
        ...
        if ( is_array( $gc_data[$root]['buyer-billing-address']['structured-name'] ) ) {
          $firstname = $gc_data[$root]['buyer-billing-address']['structured-name']['first-name']['VALUE'];
          $lastname = $gc_data[$root]['buyer-billing-address']['structured-name']['last-name']['VALUE'];
        }else{
          list ($firstname, $lastname) = 
            explode(' ', gc_makeSqlString($gc_data[$root]['buyer-billing-address']['contact-name']['VALUE']), 2);
        }
        //$group = (defined('DEFAULT_USER_LOGIN_GROUP')?(int)DEFAULT_USER_LOGIN_GROUP:0);
...
// {{
        $buyer_country = $gc_data[$root]['buyer-billing-address']['country-code']['VALUE'];
        $country_answer = tep_db_fetch_array(tep_db_query("select countries_id from " .
        TABLE_COUNTRIES . " where countries_iso_code_2 = '" . tep_db_input($buyer_country) . "'"));
// }}
        $buyer_state = $gc_data[$root]['buyer-shipping-address']['region']['VALUE']; 
        $buyer_zone_id = 0;
        
        $zone_answer = tep_db_fetch_array(tep_db_query("select zone_id, zone_country_id, zone_name from " .
        TABLE_ZONES . " where zone_code = '" . tep_db_input($buyer_state) . "' ".((int)$country_answer['countries_id']>0?"AND zone_country_id='".(int)$country_answer['countries_id']."'":"") ));
        if ( is_array($zone_answer) ) {
          $buyer_zone_id = $zone_answer['zone_id'];
          $buyer_state = $zone_answer['zone_name'];
        }
        if ( is_array( $gc_data[$root]['buyer-shipping-address']['structured-name'] ) ) {
          $firstname = $gc_data[$root]['buyer-shipping-address']['structured-name']['first-name']['VALUE'];
          $lastname = $gc_data[$root]['buyer-shipping-address']['structured-name']['last-name']['VALUE'];
        }else{
          list ($firstname, $lastname) = 
        ...
        $sql_data_array = array (
          ...
          'entry_country_id' => $country_answer['countries_id'],
          'entry_zone_id' => $buyer_zone_id
        );
        ...
        $customer_country_id = $country_answer['countries_id'];
        $customer_zone_id = $zone_answer['zone_id'];
      } else {
      	$address_book_array = tep_db_fetch_array($address_book);
        $customer_default_address_id = $address_book_array['address_book_id'];
        $customer_country_id = $address_book_array['entry_country_id'];
        $customer_zone_id = $address_book_array['entry_zone_id'];
      }
...
//  Create the order 
      ...
      $shipping_class = $shipping_name.'_'.$shipping_code;
      ...
      $sess_cart = false;
      if(isset($gc_data[$root]['shopping-cart']['merchant-private-data']['sess_cart']['VALUE'])) {
        $sess_cart = unserialize(base64_decode($gc_data[$root]['shopping-cart']['merchant-private-data']['sess_cart']['VALUE']));
      }
      if (is_object($sess_cart)) {
        $cart = $sess_cart;
      }
...
//      }
      //    Set up order info
      // patch correct country data
      $_bill_country = array('title' => $gc_data[$root]['buyer-billing-address']['country-code']['VALUE'],
                             'format_id'=>2);
      $_r_co = tep_db_query("select countries_id, countries_name, countries_iso_code_2, countries_iso_code_3, address_format_id from " . TABLE_COUNTRIES . " where countries_iso_code_2 = '" . tep_db_input($gc_data[$root]['buyer-billing-address']['country-code']['VALUE']) . "' and language_id = '" .(int)$languages_id. "'");
      if ( $_co = tep_db_fetch_array($_r_co) ) {
        $_bill_country = array('id' => $_co['countries_id'], 
                               'title' => $_co['countries_name'], 
                               'iso_code_2' => $_co['countries_iso_code_2'], 
                               'iso_code_3' => $_co['countries_iso_code_3'],
                               'format_id' => $_co['address_format_id']);
      }
      $_delivery_country = array('title' => $gc_data[$root]['buyer-shipping-address']['country-code']['VALUE'],
                                 'format_id'=>2);
      $_r_co = tep_db_query("select countries_id, countries_name, countries_iso_code_2, countries_iso_code_3, address_format_id from " . TABLE_COUNTRIES . " where countries_iso_code_2 = '" . tep_db_input($gc_data[$root]['buyer-shipping-address']['country-code']['VALUE']) . "' and language_id = '" .(int)$languages_id. "'");
      if ( $_co = tep_db_fetch_array($_r_co) ) {
        $_delivery_country = array('id' => $_co['countries_id'], 
                                   'title' => $_co['countries_name'], 
                                   'iso_code_2' => $_co['countries_iso_code_2'], 
                                   'iso_code_3' => $_co['countries_iso_code_3'],
                                   'format_id' => $_co['address_format_id']);
      ...
      //{{
      $_billing_zone_id = 0;
      $_billing_state = $gc_data[$root]['buyer-billing-address']['region']['VALUE'];
      if ( $_bill_country['id']>0 ) {
        $_look_zone = tep_db_fetch_array(tep_db_query("select zone_id, zone_country_id, zone_name from " .
        TABLE_ZONES . " where zone_code = '" . tep_db_input($_billing_state) . "' AND zone_country_id='".(int)$_bill_country['id']."'"));
        if ( is_array($_look_zone) ) {
          $_billing_zone_id = $_look_zone['zone_id'];
          $_billing_state = $_look_zone['zone_name'];
        }
      }
      $_delivery_zone_id = 0;
      $_delivery_state = $gc_data[$root]['buyer-shipping-address']['region']['VALUE'];
      if ( $_delivery_country['id']>0 ) {
        $_look_zone = tep_db_fetch_array(tep_db_query("select zone_id, zone_country_id, zone_name from " .
        TABLE_ZONES . " where zone_code = '" . tep_db_input($_delivery_state) . "' AND zone_country_id='".(int)$_delivery_country['id']."'"));
        if ( is_array($_look_zone) ) {
          $_delivery_zone_id = $_look_zone['zone_id'];
          $_delivery_state = $_look_zone['zone_name'];
        }
      }
     //}}
      ...
      if ( is_array( $gc_data[$root]['buyer-billing-address']['structured-name'] ) ) {
        $order->customer['firstname'] = $gc_data[$root]['buyer-billing-address']['structured-name']['first-name']['VALUE'];
        $order->customer['lastname'] = $gc_data[$root]['buyer-billing-address']['structured-name']['last-name']['VALUE'];
      }else{
      ...
      $order->customer['state'] = $_billing_state;
      $order->customer['zone_id'] = $_billing_zone_id;
      $order->customer['country'] = $_bill_country;
      ...
      $order->customer['format_id'] = $_bill_country['format_id'];
      if ( is_array( $gc_data[$root]['buyer-shipping-address']['structured-name'] ) ) {
        $order->delivery['firstname'] = $gc_data[$root]['buyer-shipping-address']['structured-name']['first-name']['VALUE'];
        $order->delivery['lastname'] = $gc_data[$root]['buyer-shipping-address']['structured-name']['last-name']['VALUE'];
      }else{
      ...
      $order->delivery['state'] = $_delivery_state;
      $order->delivery['zone_id'] = $_delivery_zone_id;
      $order->delivery['country'] = $_delivery_country;
      $order->delivery['format_id'] = $_delivery_country['format_id'];
      if ( is_array( $gc_data[$root]['buyer-billing-address']['structured-name'] ) ) {
        $order->billing['firstname'] = $gc_data[$root]['buyer-billing-address']['structured-name']['first-name']['VALUE'];
        $order->billing['lastname'] = $gc_data[$root]['buyer-billing-address']['structured-name']['last-name']['VALUE'];
      }else{
      ...
      $order->billing['zone_id'] = $_billing_zone_id;
      $order->billing['country'] = $_bill_country;
      $order->billing['format_id'] = $_bill_country['format_id'];
      ...
      $order->info['shipping_class'] = $shipping_class;
      $order->info['cc_type'] = '';
      $order->info['cc_owner'] = '';
...
//      $gift_cert = get_arr_result(@$gc_data[$root]['order-adjustment']['merchant-codes']['gift-certificate-adjustment']);
      ...
      $tax_addr = array(
        'c' => $order->billing['country']['id'],
        'z' => $order->billing['zone_id'],
      );
      if ( (int)$order->delivery['country']['id']>0 ) {
        $tax_addr = array(
          'c' => $order->delivery['country']['id'],
          'z' => $order->delivery['zone_id'],
        );
      }
      foreach ($items as $item) {
        if (isset ($item['merchant-private-item-data']['item']['VALUE'])) {
          $_order_products = unserialize(base64_decode($item['merchant-private-item-data']['item']['VALUE']));
          $tax_class_array=tep_db_fetch_array(tep_db_query("select products_tax_class_id as class_id from " . TABLE_PRODUCTS . " where products_id='".intval($_order_products['id'])."'"));
          if ( (int)$tax_class_array['class_id']>0 ) {
            $_order_products['tax'] = tep_get_tax_rate( (int)$tax_class_array['class_id'], $tax_addr['c'], $tax_addr['z'] );
          }
          $product_vat_corr += tep_add_tax( $_order_products['final_price'], $_order_products['tax'] );
          $order->products[] = $_order_products;
        } else
      ...
      $_f_ot_post_process = $order_total_modules->modules;
      ...
      $order_total_modules->modules = $_f_ot_post_process;
...
if ($file = @fopen(dirname(__FILE__)."/logs/common.log", "a")){
  $request = isset($HTTP_RAW_POST_DATA)?$HTTP_RAW_POST_DATA:file_get_contents("php://input");
  @fwrite($file, "OUT At ".date('Y-m-d H:i:s').' ['.$root.']'."\n");
  @fclose($file);
}
...
function process_merchant_calculation_callback_single($Gresponse) {
  global $googlepayment, $order, $cart, $total_weight, $total_count, $customer_groups_id;
...
//  $cartID = $cart->cartID;
  ...
  foreach($addresses as $curr_address) {
    ...
    $row = tep_db_fetch_array(tep_db_query("select * from ". TABLE_COUNTRIES ." where countries_iso_code_2 = '". gc_makeSqlString($country) ."' and language_id = 1"));
...
//            Compute the price for this shipping method and address id
        ...
        if( isset($quotes[$quote_povider]['error']) ||
            ...
            !is_array($quotes[$quote_povider]) ) {
        	$price = "9999.09";
        	$shippable = "false";
        } else {
        	$price = $quotes[$quote_povider]['methods'][$quote_method]['cost'];
        	if ( isset($coupon_type) && $coupon_type=='S' ) $price = '0.00';
        	$shippable = "true";
        }
