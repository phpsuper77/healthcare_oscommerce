...
for ($i = 0, $n = sizeof($products); $i < $n; $i++) {
  $product_virtual = false;  
	...
	if ( !is_array($tax) ) {
	  $tax = array('tax_class_title'=>'none');
  }
...
$__coupon_present = false;
foreach($order_totals as $order_total){
  if ($order_total['code']=='ot_coupon') $__coupon_present = true;
}
if ( $__coupon_present==false && (int)$cc_id!=0 && $coupon_type=='S' ) {
  $__ship_coupon = true;
}else{
  $__ship_coupon = false;
}
if ( $__ship_coupon ){
  $_order_totals = $order_totals;
  $order_totals = array();
  foreach($_order_totals as $ot){
    if ( $ot['code']=='ot_total' && is_object($GLOBALS['ot_coupon']) ) {
      $order_totals[] = array('code'=>'ot_coupon','title' => $GLOBALS['ot_coupon']->title . ':' . $GLOBALS['ot_coupon']->coupon_code .':'.TEXT_FREE_SHIPPING,'text'=>'0.0','value'=>'0.00');
    }
    $order_totals[] = $ot;
  }
}
...
// Cant used this since the OT is passed as an item, and tax cant be calculated
    ...
    $ot_value = abs($order_total['value']) * (strrpos($order_total['text'], '-')===false?1:-1);//($order_total['text']{0}=='-'?-1:1);
...
//                            $currencies->get_value(DEFAULT_CURRENCY) * $ot_value);
    $_item_keep_data = array('order_total' => base64_encode(serialize($order_total))
                             );
    $Gitem->SetMerchantPrivateItemData(
            new MerchantPrivateItemData($_item_keep_data));
    if(!empty($tt)) {
      $Gitem->SetTaxTableSelector($tt);
    }
...
$_order_keep_data = array('session-data' => $private_data,
                          'search_engines_id' => (int)$search_engines_id,
                          'search_words_id' => (int)$search_words_id,
                          'customer_groups_id' => $customer_groups_id,
                          'sess_cart' => base64_encode(serialize($cart)),
                          'cc_id'=>$cc_id,
                          'coupon_type'=>$coupon_type,
                         );

$Gcart->SetMerchantPrivateData(
               new MerchantPrivateData($_order_keep_data));
...
if(DOWNLOAD_ENABLED != 'true' || $cart->get_content_type() != 'virtual') {
  ...
  $module_directory = DIR_FS_CATALOG . (substr(DIR_FS_CATALOG,-1)=='/'?'':'/') . DIR_WS_MODULES . 'shipping/';
  ...
  $check_query = tep_db_fetch_array(tep_db_query("select countries_iso_code_2 
                               ...
                               where countries_id = 
                               '" . SHIPPING_ORIGIN_COUNTRY . "' and language_id = 1"));
  ...
  for ($i = 0, $n = sizeof($directory_array); $i < $n; $i++) {
  	...
  	include_once (DIR_FS_CATALOG . (substr(DIR_FS_CATALOG, -1)=='/'?'':'/') .DIR_WS_LANGUAGES . $language . '/modules/shipping/' . $file);
    ...
    if ($enable == "True") {
    	if ($zone != '') {
    		$zone_result = tep_db_query("SELECT countries_name, coalesce(zone_code, 'All Areas') zone_code, countries_iso_code_2
                                      ...
                                      inner join ". TABLE_COUNTRIES ." AS c on ztgz.zone_country_id = c.countries_id and c.language_id = 1
...
$wgProducts = '';
$porPTotal = 0;
$_cart_products = $cart->get_products();

if (sizeof($_cart_products) > 0) {
  $porPD = array();
  foreach($_cart_products as $porP) {
    $porPrice = number_format((tep_add_tax($porP['final_price'],tep_get_tax_rate($porP['tax_class_id']))*$porP['quantity']),2,'.','');
    $porPD[] = '8995' . '::' . $porPrice . '::' . preg_replace("/[^a-zA-Z0-9\.\,()]+/"," ",strip_tags($porP['name'])) . '::' . $porP['id'];
    $porPTotal +=$porPrice;
  }
  $wgProducts = rawurlencode(implode('|', $porPD));
} 

$Gcart->AddThirdPartyTracking(
  'https://track.webgains.com/transaction.html?wgver=1.2&wgrs=1&wgsubdomain=track&wglang='.$wgLang.'&wgeventid='.$wgEventID.'&wgprogramid='.$wgProgramID.'&wgvalue='.$porPTotal.'&wgitems='.$wgProducts,
  array(
...
foreach ($tax_array as $tax_table) {
  ...
  if ( $num_rows!=0 ) {
    ...
    $GAtaxRule = new GoogleAlternateTaxRule('0.00');
    $GAtaxRule->SetWorldArea(true);
    $GAtaxTable->AddAlternateTaxRules($GAtaxRule);
  }
	$i++;
