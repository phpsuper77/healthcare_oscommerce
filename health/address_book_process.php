<?php
/*
  $Id: address_book_process.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
// error checking when updating or adding an entry
  ...
  if (isset($HTTP_POST_VARS['action']) && (($HTTP_POST_VARS['action'] == 'process') || ($HTTP_POST_VARS['action'] == 'update'))) {
    ...
    $entry_company_vat = tep_db_prepare_input($HTTP_POST_VARS['entry_company_vat']);
    ...
    if (ACCOUNT_COMPANY_VAT_ID == 'true'){
        ...
        $error = true;
        $messageStack->add('addressbook', ENTRY_BUSINESS_ERROR);
      }
      if (check_customer_groups($group, 'groups_is_reseller') && empty($entry_company_vat)){
        $error = true;
        $messageStack->add('addressbook', ENTRY_BUSINESS_COMPANY_ERROR);
      }
    }
...
// reregister session variables
  ...
  if (isset($HTTP_GET_VARS['edit']) && is_numeric($HTTP_GET_VARS['edit'])) {
    $entry_query = tep_db_query("select entry_gender, entry_company, entry_firstname, entry_lastname, entry_street_address, entry_suburb, entry_postcode, entry_city, entry_state, entry_zone_id, entry_country_id, entry_company_vat from " . TABLE_ADDRESS_BOOK . " where customers_id = '" . (int)$customer_id . "' and address_book_id = '" . (int)$HTTP_GET_VARS['edit'] . "'");
