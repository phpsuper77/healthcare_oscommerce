<?php
/*
  $Id: affiliate_signup.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
*/
  ...
  if (isset($HTTP_POST_VARS['action'])) {
    ...
    if (strlen($a_from_email_address) < ENTRY_EMAIL_ADDRESS_MIN_LENGTH) {
      $error = true;
      $entry_from_email_address_error = true;
    } else {
      $entry_from_email_address_error = false;
    }

    if (!tep_validate_email($a_from_email_address)) {
      $error = true;
      $entry_from_email_address_check_error = true;
    } else {
      $entry_from_email_address_check_error = false;
    }
    ...
    if (!$error) {
      ...
      $sql_data_array = array('affiliate_firstname' => $a_firstname,
                              ...
                              'affiliate_email_from' => $a_from_email_address,
                              'affiliate_store_name' => $a_store_name,
                              'affiliate_agb' => '1');
      ...
      $affiliate_id = affiliate_insert ($sql_data_array, (AFFILIATE_EXTRA != 'True'?$HTTP_SESSION_VARS['aff_ref']:$HTTP_SESSION_VARS['affiliate_ref']) );

      $aemailbody = MAIL_AFFILIATE_HEADER . "\n"
                  . MAIL_AFFILIATE_ID . $affiliate_id . "\n"
                  . MAIL_AFFILIATE_USERNAME . tep_get_clickable_link($a_email_address) . "\n"
                  . MAIL_AFFILIATE_PASSWORD . $a_password . "\n\n"
                  . MAIL_AFFILIATE_LINK . ' '
                  . tep_get_clickable_link(tep_href_link(FILENAME_LOGIN, 'type=affiliate')) . "\n\n"
