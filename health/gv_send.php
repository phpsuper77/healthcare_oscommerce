<?php
/*
  $Id: gv_send.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
// if the customer is not logged on, redirect them to the login page
  ...
  if ($HTTP_GET_VARS['action'] == 'process') {
    ...
    if ($new_amount<0) {
      ...
      $gv_query=tep_db_query("update " . TABLE_COUPON_GV_CUSTOMER . " set amount = '" . tep_db_input($new_amount) . "' where customer_id = '" . (int)$customer_id . "'");
      ...
      $gv_query=tep_db_query("insert into " . TABLE_COUPONS . " (coupon_type, coupon_code, date_created, coupon_amount) values ('G', '" . $id1 . "', NOW(), '" . tep_db_input($HTTP_POST_VARS['amount']) . "')");
      $insert_id = tep_db_insert_id($gv_query);
      $gv_query=tep_db_query("insert into " . TABLE_COUPON_EMAIL_TRACK . " (coupon_id, customer_id_sent, sent_firstname, sent_lastname, emailed_to, date_sent) values ('" . (int)$insert_id . "' ,'" . (int)$customer_id . "', '" . tep_db_input($gv_customer['customers_firstname']) . "', '" . tep_db_input($gv_customer['customers_lastname']) . "', '" . tep_db_input($HTTP_POST_VARS['email']) . "', now())");
      ...
      $gv_email .= EMAIL_GV_LINK . tep_get_clickable_link(tep_href_link(FILENAME_GV_REDEEM, 'gv_no=' . $id1,'NONSSL',false));
      $gv_email .= "\n\n";  
