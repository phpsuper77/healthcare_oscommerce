<?php
/*
  $Id: checkout_payment_address.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
// process a new billing address
    if (tep_not_null($HTTP_POST_VARS['firstname']) && tep_not_null($HTTP_POST_VARS['lastname']) && tep_not_null($HTTP_POST_VARS['street_address_line1'])) {
      $process = true;
      ...
      if (ACCOUNT_COMPANY_VAT_ID == 'true'){
          ...
          $error = true;
          $messageStack->add('checkout_address', ENTRY_BUSINESS_ERROR);
        }
        if (check_customer_groups($group, 'groups_is_reseller') && empty($entry_company_vat)){
          $error = true;
          $messageStack->add('checkout_address', ENTRY_BUSINESS_COMPANY_ERROR);
        }
      }
...
// process the selected billing destination
      ...
      $billto = (int)$HTTP_POST_VARS['address'];
