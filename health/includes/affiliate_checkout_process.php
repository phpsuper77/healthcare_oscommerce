<?php
/*
  $Id: affiliate_checkout_process.php,v 1.1.1.1 2005/12/03 21:36:10 max Exp $
...
*/
  ...
  if ($HTTP_SESSION_VARS['aff_ref'] || $HTTP_SESSION_VARS['affiliate_ref']){
    if ($HTTP_SESSION_VARS['aff_ref']) $affiliate_ref = $HTTP_SESSION_VARS['aff_ref'];
  ...
  // Check for individual commission
    ...
    if (AFFILATE_INDIVIDUAL_PERCENTAGE == 'true') {
      $affiliate_commission_query = tep_db_query ("select affiliate_commission_percent from " . TABLE_AFFILIATE . " where affiliate_id = '" . (int)$affiliate_ref . "'");
    ...
    if ($affiliate_percent == 0) $affiliate_percent = AFFILIATE_PERCENT;
    ...
    if (AFFILATE_USE_TIER == 'true') {
      $affiliate_tiers_query = tep_db_query ("SELECT aa2.affiliate_id, (aa2.affiliate_rgt - aa2.affiliate_lft) as height
                                                      ...
                                                      WHERE  aa1.affiliate_root = aa2.affiliate_root 
                                                            ...
                                                            AND aa1.affiliate_id =  '" . (int)$affiliate_ref . "'
