<?php
/*
  $Id: ot_gv.php,v 1.1.1.1 2005/12/03 21:36:11 max Exp $
  $Id: ot_gv.php,v 1.1.1.1 2005/12/03 21:36:11 max Exp $
...
//        $gv_order_amount += 0.001;
        ...
        if (MODULE_ORDER_TOTAL_GV_QUEUE == 'false') {
          ...
          if ($customer_gv) {
            $gv_update=tep_db_query("update " . TABLE_COUPON_GV_CUSTOMER . " set amount = '" . tep_db_input($total_gv_amount) . "' where customer_id = '" . (int)$customer_id . "'");
          } else {
            $gv_insert=tep_db_query("insert into " . TABLE_COUPON_GV_CUSTOMER . " (customer_id, amount) values ('" . (int)$customer_id . "', '" . tep_db_input($total_gv_amount) . "')");
          }
        } else {
         // GV_QUEUE is true - so queue the gv for release by store owner
          $gv_insert=tep_db_query("insert into " . TABLE_COUPON_GV_QUEUE . " (customer_id, order_id, amount, date_created, ipaddr) values ('" . (int)$customer_id . "', '" . (int)$insert_id . "', '" . tep_db_input($gv_order_amount) . "', NOW(), '" . tep_db_input($REMOTE_ADDR) . "')");
        }
      }
    }
...
//        $image_submit = '<input type="image" name="submit_redeem" onClick="submitFunction()" src="' . DIR_WS_LANGUAGES . $language . '/images/buttons/button_redeem.gif" border="0" alt="' . IMAGE_REDEEM_VOUCHER . '" title = "' . IMAGE_REDEEM_VOUCHER . '">';
    ...
    function apply_credit() {
      ...
      if (tep_session_is_registered('cot_gv')) {
        ...
        $gv_update = tep_db_query("update " . TABLE_COUPON_GV_CUSTOMER . " set amount = '" . tep_db_input($gv_amount) . "' where customer_id = '" . (int)$customer_id . "'");
      }
      return $gv_payment_amount;
    }
      ...
      if ( is_array($result) ) {
        if (ONE_PAGE_CHECKOUT == 'True') {
          if (defined('ONE_PAGE_SHOW_TOTALS') && ONE_PAGE_SHOW_TOTALS=='true') {
            ...
            $messageStack->add('one_page_checkout', $result['message'] );
            $error = true;
          }
          if ( $result['error'] ) {
            $error = true;
          ...
          tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode($result['message']), 'SSL'));
        }
      }
    }
    ...
    function _collect_posts() {
      ...
      if ($HTTP_POST_VARS['gv_redeem_code']) {
        ...
        if ($gv_result['coupon_type'] == 'G') {
          if ( defined('ONE_PAGE_SHOW_TOTALS') && ONE_PAGE_SHOW_TOTALS=='true' && !tep_session_is_registered('customer_id') ) {
          ...
          unset($GLOBALS['gv_redeem_code']); unset($_POST['gv_redeem_code']); unset($HTTP_POST_VARS['gv_redeem_code']);
          return array('error'=>false,'message'=>ERROR_REDEEMED_AMOUNT. $currencies->format($gv_amount));
        }
      }
      if ($HTTP_POST_VARS['submit_redeem_x'] && $gv_result['coupon_type'] == 'G'){
...
//          $order->info['total'] -= $tod_amount;   //// ????? Strider
    ...
    function user_has_gv_account($c_id) {
      $gv_query = tep_db_query("select amount from " . TABLE_COUPON_GV_CUSTOMER . " where customer_id = '" . (int)$c_id . "'");
      if ($gv_result = tep_db_fetch_array($gv_query)) {
