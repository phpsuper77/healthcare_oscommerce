<?php
/*
$Id: ot_coupon.php,v 1.1.1.1 2005/12/03 21:36:11 max Exp $
$Id: ot_coupon.php,v 1.1.1.1 2005/12/03 21:36:11 max Exp $
$Id: ot_coupon.php,v 1.1.1.1 2005/12/03 21:36:11 max Exp $
...
//  $image_submit = '<input type="image" name="submit_redeem" onClick="submitFunction()" src="' . DIR_WS_LANGUAGES . $language . '/images/buttons/button_redeem.gif" border="0" alt="' . IMAGE_REDEEM_VOUCHER . '" title = "' . IMAGE_REDEEM_VOUCHER . '">';
  ...
  $selection_string .= ' <td align="right">' . tep_draw_input_field('gv_redeem_code') . '</td>';
  $selection_string .= ' <td width="10">' . tep_draw_separator('pixel_trans.gif', '10', '1') . '</td>';
  $selection_string .= '</tr>' . "\n";
  $selection_string .= '<tr>' . "\n";
  $selection_string .= ' <td width="10">' . tep_draw_separator('pixel_trans.gif', '10', '1') .'</td>';
  $selection_string .= ' <td class="main"'.((defined('ONE_PAGE_SHOW_TOTALS') && ONE_PAGE_SHOW_TOTALS=='true')?' id="label_coupon"':'').'>' . "\n";
  $selection_string .= '&nbsp;</td>';
...
// get some info from the coupon table
  ...
  if ($coupon_result['coupon_type'] != 'G') {
    ...
    if ( defined('ONE_PAGE_CALCULATE') && defined('ONE_PAGE_SHOW_TOTALS') && ONE_PAGE_SHOW_TOTALS=='true' ) {
        ...
        $valid_product = false;
          ...
          if ($get_result['restrict_to_categories']) {
            ...
            for ($c = 0; $c < sizeof($cat_ids); $c++) {
              $cat_query = tep_db_query("select products_id from ".TABLE_PRODUCTS_TO_CATEGORIES." where products_id = '" . (int)$t_prid . "' and categories_id = '" . (int)$cat_ids[$i] . "'");
              if (tep_db_num_rows($cat_query) !=0 ) $valid_product = true;
            }
          }
          if ($valid_product) {
            $price_excl_vat = $products[$i]['final_price'] * $products[$i]['quantity']; //Fred - added
            $price_incl_vat = $this->product_price($products[$i]['id']); //Fred - added
...
global $insert_id, $customer_id, $REMOTE_ADDR, $cc_id;
  //$cc_id = $_SESSION['cc_id']; //Fred commented out, do not use $_SESSION[] due to backward comp. Reference the global var instead.
  if ($this->deduction !=0) {
    tep_db_query("insert into " . TABLE_COUPON_REDEEM_TRACK . " (coupon_id, redeem_date, redeem_ip, customer_id, order_id) values ('" . (int)$cc_id . "', now(), '" . tep_db_input($REMOTE_ADDR) . "', '" . (int)$customer_id . "', '" . (int)$insert_id . "')");
  }
  tep_session_unregister('cc_id');
}
...
global $cart, $order;
  ...
  if ($product = tep_db_fetch_array($product_query)) {
    ...
    $products_price = tep_get_products_price($prid, $qty, $product['products_price']);
...
//      echo("total price = " . $total_price . " products_price = " . $products_price . " products_tax = " . $products_tax . "<br>");
    ...
    if (isset($cart->contents[$product_id]['attributes'])) {
      ...
      while (list($option, $value) = each($cart->contents[$product_id]['attributes'])) {
        $attribute_price_query = tep_db_query("select products_attributes_id, options_values_price, price_prefix from " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id = '" . (int)$prid . "' and options_id = '" . (int)$option . "' and options_values_id = '" . (int)$value . "'");
