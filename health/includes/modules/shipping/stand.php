...
// END OF INSURANCE CHARGES
      ...
      if ($dest_zone == 0) {
        ...
        $zones_cart_cost = constant('MODULE_SHIPPING_STAND_AMOUNTCOST_' . $dest_zone);
        if ( !empty($zones_cart_cost) ) {
          ...
          for ($i=0; $i<sizeof($zones_cart_table); $i+=2) {
            if ($_cart_total >= $zones_cart_table[$i]) {
			  ...
			  $shipping_method = MODULE_SHIPPING_STAND_TEXT_WAY;
            }
          }
        }
        if ($shipping == -1)  {
 ...
 $error = true;
    ...
    function install() {
      ...
      for ($i = 1; $i <= $this->num_zones; $i++) {
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_STAND_COST_" . $i ."', '1:3.85,1.5:4.95,2:5.31,4:7.7,6:8.74,8:9.97,10:10.7,20:12.46,			
', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights. Example: 3:8.50,7:10.50,... Weights less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.', '6', '0', now())");
    ...
    function keys() {
      ...
      for ($i=1; $i<=$this->num_zones; $i++) {
        ...
        $keys[] = 'MODULE_SHIPPING_STAND_AMOUNTCOST_' . $i;
