...
/*
  ...
  units are not REGified in this explanation since they should be
  REGific to your locale.
...
// END OF ADDITIONAL INSURANCE CHARGES 
      ...
      if ($dest_zone == 0) {
        ...
        $zones_cart_cost = constant('MODULE_SHIPPING_REG_AMOUNTCOST_' . $dest_zone);
        if ( !empty($zones_cart_cost) ) {
          ...
          for ($i=0; $i<sizeof($zones_cart_table); $i+=2) {
            if ($_cart_total >= $zones_cart_table[$i]) {
              ...
              $shipping_method = 'Signature On Delivery';
            }
          }
        }
        if ($shipping == -1)  {
...
// MAXIMUM VALUE OF ORDER FOR RECORED - EDIT AS NECESSARY
if ($order->info['total'] > 250&&false) {
 $error = true;
         ...
         $shipping_cost = ($shipping * $shipping_num_boxes) + $ic + constant('MODULE_SHIPPING_REG_HANDLING_' . $dest_zone);
    ...
    function install() {
      ...
      for ($i = 1; $i <= $this->num_zones; $i++) {
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_REG_COST_" . $i ."', '.06:1,.1:1.17,.15:1.36,.2:1.52,.25:1.67,.3:1.82,.35:1.97,.4:2.17,.45:2.38,.5:2.58,.6:2.98,.7:3.37,.75:3.57,.8:3.78,.9:4.17,1:4.57,									
...
', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights. Example: 3:8.50,7:10.50,... Weights less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.', '6', '0', now())");
    ...
    function keys() {
      ...
      for ($i=1; $i<=$this->num_zones; $i++) {
        ...
        $keys[] = 'MODULE_SHIPPING_REG_AMOUNTCOST_' . $i;
