...
/*
  ...
  units are not FIRSTified in this explanation since they should be
  FIRSTific to your locale.
...
// END OF ADDITIONAL INSURANCE CHARGES 
      ...
      if ($dest_zone == 0) {
        ...
        $shipping = -1;
        // cart amount first in check - free for > 50 pounds
        $_cart_total = $order->info['total'];
        $zones_cart_cost = constant('MODULE_SHIPPING_FIRST_AMOUNTCOST_' . $dest_zone);
        if ( !empty($zones_cart_cost) ) {
          $zones_cart_table = split("[:,]" , $zones_cart_cost);
          for ($i=0; $i<sizeof($zones_cart_table); $i+=2) {
            if ($_cart_total >= $zones_cart_table[$i]) {
              $shipping = $zones_cart_table[$i+1];
              $shipping_method = 'Low Value Orders';
            }
          }
        }
        if ($shipping == -1)  {
...
// MAXIMUM VALUE OF ORDER FOR RECORDED - EDIT AS NECESSARY
if ($order->info['total'] > 250&&false) {
 $error = true;


      } else {

         $shipping_cost = ($shipping * $shipping_num_boxes) + $ic + constant('MODULE_SHIPPING_FIRST_HANDLING_' . $dest_zone);
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_FIRST_STATUS'");
    ...
    function install() {
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable Zones Method', 'MODULE_SHIPPING_FIRST_STATUS', 'True', 'Do you want to offer zone rate shipping?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Tax Class', 'MODULE_SHIPPING_FIRST_TAX_CLASS', '0', 'Use the following tax class on the shipping fee.', '6', '0', 'tep_get_tax_class_title', 'tep_cfg_pull_down_tax_classes(', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_SHIPPING_FIRST_SORT_ORDER', '0', 'Sort order of display.', '6', '0', now())");
      for ($i = 1; $i <= $this->num_zones; $i++) {
        $default_countries = '';
        if ($i == 1) {
          $default_countries = 'GB';
        }
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Countries', 'MODULE_SHIPPING_FIRST_COUNTRIES_" . $i ."', '" . $default_countries . "', 'Comma separated list of two character ISO country codes that are part of Zone " . $i . ".', '6', '0', now())");
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_FIRST_COST_" . $i ."', '.06:.32,.1:.49,.15:.68,.2:.84,.25:.99,.3:1.14,.35:1.29,.4:1.49,.45:1.7,.5:1.9,.6:2.3,.7:2.69,.75:2.89,.8:3.10,.9:3.49,1:3.89,1.25:4.74,1.5:5.59,1.75:6.44,2:7.29											
...
', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights. Example: 3:8.50,7:10.50,... Weights less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.', '6', '0', now())");
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Handling Fee', 'MODULE_SHIPPING_FIRST_HANDLING_" . $i."', '0', 'Handling Fee for this shipping zone', '6', '0', now())");
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Amount Table', 'MODULE_SHIPPING_FIRST_AMOUNTCOST_" . $i ."', '50:0', 'Shipping rates to Zone " . $i . " destinations based on a order amount. Example: 3:8.50,7:10.50,... Amount more than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.', '6', '0', now())");
    ...
    function keys() {
      $keys = array('MODULE_SHIPPING_FIRST_STATUS', 'MODULE_SHIPPING_FIRST_TAX_CLASS', 'MODULE_SHIPPING_FIRST_SORT_ORDER');

      for ($i=1; $i<=$this->num_zones; $i++) {
        $keys[] = 'MODULE_SHIPPING_FIRST_COUNTRIES_' . $i;
        $keys[] = 'MODULE_SHIPPING_FIRST_AMOUNTCOST_' . $i;
        $keys[] = 'MODULE_SHIPPING_FIRST_COST_' . $i;
        $keys[] = 'MODULE_SHIPPING_FIRST_HANDLING_' . $i;
      }
