...
$ic = 0;
 if ($order->info['total'] > 500) $ic = .45;
 if ($order->info['total'] > 1000) $ic = 1.35;
...
// END OF ADDITIONAL INSURANCE CHARGES 
      ...
      if ($dest_zone == 0) {
        ...
        $zones_cart_cost = constant('MODULE_SHIPPING_SPEC_AMOUNTCOST_' . $dest_zone);
        if ( !empty($zones_cart_cost) ) {
          ...
          for ($i=0; $i<sizeof($zones_cart_table); $i+=2) {
            if ($_cart_total >= $zones_cart_table[$i]) {
              ...
              $shipping_method = 'Urgent or higher value - Insurance Included Upto £2500'; 
			 //$shipping_method = 'Urgent or higher value - Insurance Included'; */
            }
          }
        }
        if ($shipping == -1)  {
...
if ($order->info['total'] > 2500&&false) {
 $error = true;
    ...
    function install() {
      ...
      for ($i = 1; $i <= $this->num_zones; $i++) {
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_SPEC_COST_" . $i ."', '.1:4.1,.5:4.5,1:5.7,2:7.4,10:18.5,	
...
', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights. Example: 3:8.50,7:10.50,... Weights less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.', '6', '0', now())");
    ...
    function keys() {
      ...
      for ($i=1; $i<=$this->num_zones; $i++) {
        ...
        $keys[] = 'MODULE_SHIPPING_SPEC_AMOUNTCOST_' . $i;
