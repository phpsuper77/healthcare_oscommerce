...
// class constructor
    function int3() {
      $this->code = 'int3';
      $this->title = MODULE_SHIPPING_INT3_TEXT_TITLE;
      $this->description = MODULE_SHIPPING_INT3_TEXT_DESCRIPTION;
      $this->sort_order = MODULE_SHIPPING_INT3_SORT_ORDER;
      $this->icon = '';
      $this->tax_class = MODULE_SHIPPING_INT3_TAX_CLASS;

     $this->enabled = ((MODULE_SHIPPING_INT3_STATUS == 'True') ? true : false);
      ...
      if ( $this->enabled == true ) {
        ...
        for ($i=1; $i<=$this->num_zones; $i++) {
          $countries_table = constant('MODULE_SHIPPING_INT3_COUNTRIES_' . $i);
...
// END OF INSURANCE CHARGES
      ...
      if ($dest_zone == 0) {
        ...
        $shipping = -1;
        $zones_cost = constant('MODULE_SHIPPING_INT3_COST_' . $dest_zone);
      ...
      if ($shipping_weight <= $zones_table[$i]) {
        ...
        if ($shipping == -1)  {
         $shipping_cost = 0;
         $shipping_method = MODULE_SHIPPING_INT3_UNDEFINED_RATE;
$error = true;
}
...
if ($order->info['total'] > 500&&false) {
 $error = true;
         ...
         $shipping_cost = ($shipping * $shipping_num_boxes) + $ic + constant('MODULE_SHIPPING_INT3_HANDLING_' . $dest_zone);

      }
      }

      $this->quotes = array('id' => $this->code,
                            'module' => MODULE_SHIPPING_INT3_TEXT_TITLE,
      ...
      if ($error == true) $this->quotes['error'] = MODULE_SHIPPING_INT3_INVALID_ZONE;
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_INT3_STATUS'");
    ...
    function install() {
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable Zones Method', 'MODULE_SHIPPING_INT3_STATUS', 'True', 'Do you want to offer zone rate shipping?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");

      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Tax Class', 'MODULE_SHIPPING_INT3_TAX_CLASS', '0', 'Use the following tax class on the shipping fee.', '6', '0', 'tep_get_tax_class_title', 'tep_cfg_pull_down_tax_classes(', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_SHIPPING_INT3_SORT_ORDER', '0', 'Sort order of display.', '6', '0', now())");
      for ($i = 1; $i <= $this->num_zones; $i++) {
        $default_countries = '';
        if ($i == 1) {
          $default_countries = 'AS,AQ,AU,CN,CK,TP,,FJ,PF,TF,GU,JP,KI,KP,KR,MH,FM,MN,NR,NC,NZ,NU,NF,MP,PW,PG,PH,PN,WS,SB,TW,TK,TO,TV,VU,WF';
        }
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Countries', 'MODULE_SHIPPING_INT3_COUNTRIES_" . $i ."', '" . $default_countries . "', 'Comma separated list of two character ISO country codes that are part of Zone " . $i . ".', '6', '0', now())");
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_INT3_COST_" . $i ."', '0.1:1.37,0.2:2.32,0.3:3.27,0.4:4.22,0.5:5.17,0.6:6.12,0.7:7.07,0.8:8.02,0.9:8.97,1:9.92,1.1:10.82,1.2:11.72,1.3:12.62,1.4:13.52,1.5:14.42,1.6:15.32,1.7:16.22,1.8:17.12,1.9:18.02,2:18.92,', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights. Example: 3:8.50,7:10.50,... Weights less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.', '6', '0', now())");
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Handling Fee', 'MODULE_SHIPPING_INT3_HANDLING_" . $i."', '0', 'Handling Fee for this shipping zone', '6', '0', now())");
      }
    }
    ...
    function keys() {
      $keys = array('MODULE_SHIPPING_INT3_STATUS', 'MODULE_SHIPPING_INT3_TAX_CLASS', 'MODULE_SHIPPING_INT3_SORT_ORDER');

      for ($i=1; $i<=$this->num_zones; $i++) {
        $keys[] = 'MODULE_SHIPPING_INT3_COUNTRIES_' . $i;
        $keys[] = 'MODULE_SHIPPING_INT3_COST_' . $i;
        $keys[] = 'MODULE_SHIPPING_INT3_HANDLING_' . $i;
      }
