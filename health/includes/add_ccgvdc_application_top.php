...
// Update the Customers GV account
  function tep_gv_account_update($customer_id, $gv_id) {
    ...
    $coupon_gv_query = tep_db_query("select coupon_amount from " . TABLE_COUPONS . " where coupon_id = '" . (int)$gv_id . "' and coupon_active='Y'");
   ...
   // new code bugfix
   $gv_query = tep_db_query("update " . TABLE_COUPON_GV_CUSTOMER . " set amount = '" . tep_db_input($new_gv_amount) . "' where customer_id = '" . (int)$customer_id . "'");
	 // original code $gv_query = tep_db_query("update " . TABLE_COUPON_GV_CUSTOMER . " set amount = '" . $new_gv_amount . "'");
    } else {
      $gv_query = tep_db_query("insert into " . TABLE_COUPON_GV_CUSTOMER . " (customer_id, amount) values ('" . (int)$customer_id . "', '" . tep_db_input($coupon_gv['coupon_amount']) . "')");
    }
  }
////
