...
*/
  ...
  define('FILENAME_EXPRESS_CHECKOUT_IMG', DIR_WS_IMAGES . 'btn_xpressCheckout.gif');
  define('PAYPAL_WPP_DISABLE_DP', 'true');
  define('PAYPAL_WPP_DISABLE_CHECKOUT_SELECTIOIN', 'true');
...
function tep_paypal_wpp_in_ec(){
  global $ec_enabled;
  return !(!$ec_enabled || isset($_GET['ec_cancel']) || (!tep_session_is_registered('paypal_ec_payer_id') && !tep_session_is_registered('paypal_ec_payer_info')));
}
  
  function tep_paypal_wpp_ep_button($return_to = '') {
    global $ec_enabled, $PHP_SELF, $request_type;
...
?>
	<a class="checkout-paypal" href="<?php echo tep_href_link(basename($PHP_SELF), 'action=express_checkout&return_to=' . $return_to, 'SSL'); ?>"><img src="<?php echo FILENAME_EXPRESS_CHECKOUT_IMG; ?>" border=0></a>
<?php
  }
      ...
      <tr>
        <td>
             <table border="0" width="100%" cellspacing="0" cellpadding="2" class="contentBoxContents">
                  ...
                  <td align="center" class="main"><b><?php echo TEXT_PAYPALWPP_EC_SWITCH_METHOD_1; ?></b><br><a href="<?php echo tep_href_link($filename, 'ec_cancel=1', 'SSL'); ?>"><?php echo TEXT_PAYPALWPP_EC_SWITCH_METHOD_2; ?></a></td>
                          <td width="10"><?php echo tep_draw_separator('pixel_trans.gif', '10', '1'); ?></td>
                        </tr>
              		    </table>
                    </td>
...
<?php
  ...
  switch ($current_page) {
    ...
    case FILENAME_CHECKOUT_SHIPPING:
      if ($ec_enabled) {
        if ( defined('ONE_PAGE_CHECKOUT') && ONE_PAGE_CHECKOUT=='True' && tep_session_is_registered('paypal_error')) {
          $checkout_login = true;
