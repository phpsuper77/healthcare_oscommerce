<?php
/*
  $Id: banner.php,v 1.1.1.1 2005/12/03 21:36:11 max Exp $
...
// Auto activate banners
  function tep_activate_banners() {
    tep_db_query("update " . TABLE_BANNERS . " set status = '1', date_status_change = now() where date_scheduled != '' and now() >= date_scheduled and status = 0");
  }
...
// Display a banner from the specified group or banner id ($identifier)
  function tep_display_banner($action, $identifier) {
    ...
    if ($action == 'dynamic') {
      $banners_query = tep_db_query("select count(*) as count from " . TABLE_BANNERS . " where status = '1' and banners_group = '" . tep_db_input($identifier) . "' " . (isset($HTTP_SESSION_VARS['affiliate_ref'])&& $affiliate_ref != ''?" and (affiliate_id = 0 or affiliate_id = '" . (int)$affiliate_ref . "')":" and affiliate_id=0"));
      $banners = tep_db_fetch_array($banners_query);
      if ($banners['count'] > 0) {
        $banner = tep_random_select("select banners_id, banners_title, banners_image, banners_html_text, affiliate_id from " . TABLE_BANNERS . " where status = '1' and banners_group = '" . tep_db_input($identifier) . "' " . (isset($HTTP_SESSION_VARS['affiliate_ref'])&& $affiliate_ref != ''?" and (affiliate_id = 0 or affiliate_id = '" . (int)$affiliate_ref . "')":" and affiliate_id=0"));
      } else {
      ...
      if (is_array($identifier)) {
        ...
        $banner_query = tep_db_query("select banners_id, banners_title, banners_image, banners_html_text, affiliate_id from " . TABLE_BANNERS . " where status = '1' and banners_id = '" . (int)$identifier . "'");
...
// Check to see if a banner exists
  function tep_banner_exists($action, $identifier) {
    global $affiliate_ref, $HTTP_SESSION_VARS;
    if ($action == 'dynamic') {
      return tep_random_select("select banners_id, banners_title, banners_image, banners_html_text, affiliate_id from " . TABLE_BANNERS . " where status = '1' and banners_group = '" . tep_db_input($identifier) . "' " . (isset($HTTP_SESSION_VARS['affiliate_ref']) && $affiliate_ref != ''?" and (affiliate_id = 0 or affiliate_id = '" . (int)$affiliate_ref . "')":" and affiliate_id=0"));
    } elseif ($action == 'static') {
      $banner_query = tep_db_query("select banners_id, banners_title, banners_image, banners_html_text, affiliate_id from " . TABLE_BANNERS . " where status = '1' and banners_id = '" . (int)$identifier . "' " . (isset($HTTP_SESSION_VARS['affiliate_ref'])&& $affiliate_ref != ''?" and (affiliate_id = 0 or affiliate_id = '" . (int)$affiliate_ref . "')":" and affiliate_id=0"));
