...
if (!defined('RELATIVE_PATH')) define('RELATIVE_PATH',DIR_WS_CLASSES.'html2pdf/');
...
class HTML2FPDF extends FPDF
{
  ...
  //! @return An object (a class instance)
    ...
    $this->usepre=true;
    $this->debug_str="";
    $this->DefaultFont = $this->saveFont();
    $this->SavedFont = $this->saveFont();
  }
...
//      $this->debug("Test old:${ltag} <br>");
  ...
  //! @desc The footer is printed in every page!
      ...
      $this->SetY(-10);
//      $this->Cell(0,10,STORE_NAME,0,0,'R');
      //$this->Cell(0,10,lesser_entity_decode(strip_tags(FOOTER_TEXT_BODY)),0,0,'R');
      //$this->Cell(0,10,lesser_entity_decode(FOOTER_TEXT_BODY),0,0,'R');
      
      //Return Font to normal
      $this->restoreFont($this->DefaultFont);
  }
    ...
    foreach($a as $i => $e) {
      if($i%2==0) {
        ...
        //Start of 'if/elseif's
          ...
          if($this->tdbegin) {
      	    $this->cell[$this->row][$this->col]['textbuffer'][] = array($e,$this->HREF,$this->currentstyle,$this->colorarray,$this->currentfont_string,$this->SUP,$this->SUB,''/*internal link*/,$this->strike,$this->outlineparam,$this->bgcolorarray,$this->saveFont());
      	    $this->cell[$this->row][$this->col]['text'][] = $e;
//      	    $this->cell[$this->row][$this->col]['font'] = $this->saveFont();
      	  ...
      	  $this->textbuffer[] = array($e,$this->HREF,
             ...
             $this->currentfont_string,$this->SUP,$this->SUB,
...
//        $GLOBALS['debug'][][$tag]['font_content_after_writing'] = $this->debug_font();
      } else {
      	//Tag
      	if($e{0}=='/') { 
      	  $this->CloseTag(strtoupper(substr($e,1)));
//      	  $GLOBALS['debug'][][$tag]['font_after_tag_closed'] = $this->debug_font();
        } else {
      	  //Extract attributes
	        $this->GetTagAndAttrs($e,$tag,$attr);
      	  $this->OpenTag($tag,$attr);
      	  $temp = 1;
//      	  $GLOBALS['debug'][][$tag]['font_after_tag_opened'] = $this->debug_font('current');
      	}
//      	$GLOBALS['debug'][][$tag]['font_tag_whole_open_close_tag'] = $this->SavedFont;
      	$temp = 0;
      }
//      $GLOBALS['debug'][][$tag]['font_outside'] = $this->SavedFont;
...
//    debug_print_array($GLOBALS['debug'], false, 'fontsize_inc', true);
  }
  ...
  ///////////////////////////////////////////
    ...
    case 'IMG':
        ...
        if(isset($attr['SRC']))
        {
          $srcpath = DIR_FS_DOCUMENT_ROOT.(substr(DIR_FS_DOCUMENT_ROOT,-1)=='/'?'':'/').$attr['SRC'];
          ...
          if ($this->tdbegin) 
          {
            ...
            while($numberoflines) {$this->textbuffer[] = array("\n",$this->HREF,$this->currentstyle,$this->colorarray,$this->currentfont_string,$this->SUP,$this->SUB,''/*internal link*/,$this->strike,$this->outlineparam,$this->bgcolorarray);$numberoflines--;}
          } else {
            $imgborder = 0;
  		...
  		case 'BR':
  		  if($this->tablestart)
  		  {
  		    $this->cell[$this->row][$this->col]['textbuffer'][] = array("\n",$this->HREF,$this->currentstyle,$this->colorarray,$this->currentfont_string,$this->SUP,$this->SUB,''/*internal link*/,$this->strike,$this->outlineparam,$this->bgcolorarray);
      ...
      case 'SAMP':
  		case 'CODE':
      		...
      		$this->currentfont_string='courier';
    		  break;
  		case 'TEXTAREA':
    		  $this->buffer_on = true;
...
//    		$this->SetStyle('B',false);
   		$this->restoreFont($this->SavedFont);
    }
  ...
  //! @return void
    ...
    { 
      ...
      for( $j = 0 ; $j < $numcols ; $j++ ) //Columns
      {
          ...
          {
  				  $cell = &$cells[$i][$j];
//  				  if(isset($cell['font'])) $this->restoreFont($cell['font']);
...
function debug_font($mode){
  $str = '';
  if($mode == 'current'){
    if(is_array($this->CurrentFont)){
      $str = implode('|', $this->CurrentFont);
    }
  } else {
    $str = $this->SavedFont['family'] . '-' . $this->SavedFont['style'] . '-' . $this->SavedFont['sizePt'] ;
  }
  return $str;
}
