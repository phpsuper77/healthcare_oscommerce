<?php
/*
  $Id: payment.php,v 1.1.1.1 2005/12/03 21:36:11 max Exp $
...
// EOF: WebMakers.com Added: Downloads Controller
      ...
      if (tep_session_is_registered('affiliate_ref') && $HTTP_SESSION_VARS['affiliate_ref'] != '' && defined('MODULE_PAYMENT_INSTALLED_' . (int)$affiliate_ref) && tep_not_null(constant('MODULE_PAYMENT_INSTALLED_' . (int)$affiliate_ref))){
        $module_key = 'MODULE_PAYMENT_INSTALLED_' . (int)$affiliate_ref;
      }else{
...
//  ICW CREDIT CLASS Gift Voucher System Line below amended
        if (GERMAN_SITE == 'True' && !defined('ONE_PAGE_POST_PAYMENT')){
    ...
    function selection( $opc=false ) {
      ...
      if (is_array($this->modules)) {
        ...
        while (list(, $value) = each($this->modules)) {
          $class = substr($value, 0, strrpos($value, '.'));
          if ( $opc && $ec_checkout!==true ) {
            if ( $class=='googlecheckout' ) continue;
            //not show "only admin" modules
            if ( isset($GLOBALS[$class]->admin_module) && $GLOBALS[$class]->admin_module===true ) continue;
            //\not show "only admin" modules
              $selection = $GLOBALS[$class]->selection();
              if (is_array($selection)) {
                $selection['module_status'] = $GLOBALS[$class]->enabled;
