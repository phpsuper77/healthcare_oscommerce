...
function check_addresses2(theForm) {
  var error = 0;
...
?>
  if (theForm.elements['state'].type != "hidden" && theForm.elements['state'].type!="text") {
...
?>
  ...
  if (ship_state.type != "hidden" && ship_state.type!="text" ) {
...
<?php }?>
  ...
  ship_state : (opcForm.ship_state != null && opcForm.ship_state != undefined?(opcForm.ship_state.tagName.toUpperCase() == 'INPUT'?opcForm.ship_state.value:opcForm.ship_state.options[opcForm.ship_state.selectedIndex].value):''),
...
<?php if (($order->content_type != 'virtual') && ($order->content_type != 'virtual_weight') ) { ?>
        ...
        for (i=0;i<aj_result_array.length;i++){
          ...
          if (rateBill) {
            ...
            if( cRateTr.attachEvent ) {
              cRateTr.setAttribute("onmouseover", function() {rowOverEffect_ship(this)});
            } else if( cRateTr.addEventListener ) {
               var f = new Function("rowOverEffect_ship(this);");
               cRateTr.addEventListener("mouseover",f,false);
            }          
            
            if( cRateTr.attachEvent ) {
              cRateTr.setAttribute("onmouseout", function() {rowOutEffect_ship(this)});
            } else if( cRateTr.addEventListener ) {
               var f1 = new Function("rowOutEffect_ship(this);");     
               cRateTr.addEventListener("mouseout",f1,false);
            }
            if (rateid != -1) {
              var f2 = new Function("selectRowEffect_ship(this, " + rateid + ");");
              if( cRateTr.attachEvent ) {
                cRateTr.rtId = rateid;
                cRateTr.setAttribute("onclick", function() {selectRowEffect_ship(this, this.rtId)});
              }else if( cRateTr.addEventListener ){
                cRateTr.addEventListener("click", f2, false);
              }
              cRateTr.onclick = f2;
            }
          }
...
<?php if (ACCOUNT_STATE == 'true') { ?>
        if (quotes_result.responseJS.ctl_ship_state != undefined) {
          var ctl_ship_state = fastGet('ctl_ship_state');
          if ( ctl_ship_state!=false ) {
            ctl_ship_state.innerHTML = quotes_result.responseJS.ctl_ship_state;
            var ctlAttach = ctl_ship_state.firstChild;
        ...
        if (quotes_result.responseJS.ctl_state != undefined) {
          var ctl_state = fastGet('ctl_state');
          if ( ctl_state!=false ) {
            ctl_state.innerHTML = quotes_result.responseJS.ctl_state;
            var ctlAttach2 = ctl_state.firstChild;
