<?php
/*
  $Id: file_manager.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
*/
  ...
  if (strstr($current_path, '..')) $current_path = DIR_FS_CATALOG;

  if (!is_dir($current_path)) $current_path = DIR_FS_CATALOG;

  if (!ereg('^' . DIR_FS_CATALOG, $current_path)) $current_path = DIR_FS_CATALOG;

  $action = (isset($HTTP_GET_VARS['action']) ? $HTTP_GET_VARS['action'] : '');
  if (strpos(HTTP_SERVER, 'holbi.co.uk') !== false)
    $action = '';
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'save':
        ...
        echo "\$HTTP_POST_VARS['filename'] = ".$HTTP_POST_VARS['filename']."<br>";
  ...
  $in_directory = substr(substr(DIR_FS_CATALOG, strrpos(DIR_FS_CATALOG, '/')), 1);
  $current_path_array = explode('/', $current_path);
  $document_root_array = explode('/', DIR_FS_CATALOG);
  $goto_array = array(array('id' => DIR_FS_CATALOG, 'text' => "root"));
...
<?
  ...
  $header_title_additional=tep_draw_form('goto', FILENAME_FILE_MANAGER, '', 'get').tep_draw_pull_down_menu('goto', $goto_array, $current_path, 'onChange="this.form.submit();"').'</form>';
?>
...
if (strpos(HTTP_SERVER, 'holbi.co.uk') !== false || strpos(HTTP_SERVER, 'trueloaded.com') !== false){
  ?>
      <tr>
        <td>This functionality is disabled</td>
      </tr>
<?
    die;
}
