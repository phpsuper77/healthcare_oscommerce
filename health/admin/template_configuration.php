<?php
/*
  $Id: template_configuration.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
function LF($start_dir, $dest_dir)
{
  if (!is_dir($dest_dir)){
    mkdir($dest_dir,0777);
    chmod($dest_dir,0777);
  ...
  if ($handle = opendir($start_dir))
  {
    while (false !== ($file = readdir($handle)))
    {
      if($file != '.' && $file != '..' && $file != '.svn')
      {
        ...
        {
          LF($start_dir.$file.'/', $dest_dir.$file.'/');
        }
        clearstatcache();
        if (is_file($start_dir.$file))
        {
          copy($start_dir.$file, $dest_dir.$file);
          chmod($dest_dir.$file,0666);
        }
      }
    }
    closedir($handle);
...
/*
    		if (!file_exists(DIR_FS_TEMPLATES . "/" . $template_name)) {
        		mkdir(DIR_FS_TEMPLATES . "/" . $template_name, 0774);
     		}
    		if (!file_exists(DIR_FS_TEMPLATES . "/" . $template_name . '/boxes')) {
        		mkdir(DIR_FS_TEMPLATES . "/" . $template_name . '/boxes', 0774);
     		}
    		if (!file_exists(DIR_FS_TEMPLATES . "/" . $template_name . '/images')) {
        		mkdir(DIR_FS_TEMPLATES . "/" . $template_name . '/images', 0774);
     		}
*/
        LF(DIR_FS_TEMPLATES . "/" . DEFAULT_TEMPLATE . "/", DIR_FS_TEMPLATES . "/" . $template_name . "/");
        
        $query = tep_db_query("select * from " . TABLE_TEMPLATE . " where template_name = '" . DEFAULT_TEMPLATE . "'");
        $data = tep_db_fetch_array($query);
        $template_id = $data['template_id'];
        reset($data);
        $sql = "update " . TABLE_TEMPLATE . " set ";
        foreach ($data as $key => $value){
          if ($key != 'template_id' && $key != 'template_name' && $key != 'date_added'){
            $sql .= " " . $key . " = '" . tep_db_input($value) . "', ";
          }
        }
        $sql .= "template_id = '" . $cID . "' where template_id = '" . $cID . "'";
        tep_db_query($sql);
        $query = tep_db_query("select * from " . TABLE_INFOBOX_CONFIGURATION . " where template_id = '" . $template_id . "'");
...
*/
    ...
    case 'deleteconfirm':
      ...
      $data_query = tep_db_query("select * from " . TABLE_TEMPLATE . " where template_id = '" . (int)$cID . "'");
      $data = tep_db_fetch_array($data_query);
      tep_db_query("update " . TABLE_AFFILIATE . " set affiliate_template = '" . tep_db_input(DEFAULT_TEMPLATE). "' where affiliate_template = '" . tep_db_input($data['template_name']) . "'");
...
//************************************************************************************************************************
    case 'new':
      ...
      /* This is the correct way to loop over the directory. */
        while (false !== ($file = readdir($handle))) { 
    	  if(is_dir(DIR_FS_TEMPLATES . '/' . $file) && stristr($curr_templates.".,..,content,CVS,.svn", $file ) == FALSE){
	    	$dirs[] = $file;
