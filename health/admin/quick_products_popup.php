...
<?php
   ...
   // first select all categories that have 0 as parent:
      $sql = tep_db_query("SELECT c.categories_id, cd.categories_name from ".TABLE_CATEGORIES." c, ".TABLE_CATEGORIES_DESCRIPTION." cd WHERE c.parent_id = 0 AND c.categories_id = cd.categories_id AND cd.language_id = '" . $languages_id . "' and affiliate_id = 0 ");
...
//       echo $list . $all_list . '</tr></table><p>';
  ...
  if ($HTTP_POST_VARS['cat_id']) {
      ...
      // get all active prods in that specific category
       ...
       $sql2 = tep_db_query("SELECT p.products_id, p.products_status, p.products_image, pd.products_name from ".TABLE_PRODUCTS." p, ".TABLE_PRODUCTS_DESCRIPTION." pd, ".TABLE_PRODUCTS_TO_CATEGORIES." ptc where p.products_id = ptc.products_id and p.products_id = pd.products_id and p.products_status=1 and pd.language_id = '" . $languages_id . "' and ptc.categories_id = '" . $HTTP_POST_VARS['cat_id'] . "' and pd.affiliate_id = 0");
