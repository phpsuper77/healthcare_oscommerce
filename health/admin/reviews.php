<?php
/*
  $Id: reviews.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
    ...
    switch ($action) {
      case 'setflag':
        ...
        if ( ($HTTP_GET_VARS['flag'] == '0') || ($HTTP_GET_VARS['flag'] == '1') ) {
          tep_db_query("update " . TABLE_REVIEWS . " set status = '" . $HTTP_GET_VARS['flag'] . "' where reviews_id = '" . (int)$reviews_id . "'");
        }
...
?>
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td class="main"><b><?php echo ENTRY_RATING; ?></b>&nbsp;<?php echo tep_image(DIR_WS_TEMPLATE_IMAGES . 'reviews/stars_' . $rInfo->reviews_rating . '.gif', sprintf(TEXT_OF_5_STARS, $rInfo->reviews_rating)); ?>&nbsp;<small>[<?php echo sprintf(TEXT_OF_5_STARS, $rInfo->reviews_rating); ?>]</small></td>
      </tr>
      <tr>
        <td><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
      </tr>
...
?>
                ...
                <td class="dataTableContent" align="right"><?php echo tep_image(DIR_WS_TEMPLATE_IMAGES . 'reviews/stars_' . $reviews['reviews_rating'] . '.gif'); ?></td>
                <td class="dataTableContent" align="right"><?php echo tep_date_short($reviews['date_added']); ?></td>
                <td class="dataTableContent" align="center">
<?php
      if ($reviews['status'] == '1') {
        echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_REVIEWS, 'action=setflag&flag=0&page=' . $HTTP_GET_VARS['page'] . '&rID=' . $reviews['reviews_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link(FILENAME_REVIEWS, 'action=setflag&flag=1&page=' . $HTTP_GET_VARS['page'] . '&rID=' . $reviews['reviews_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
      }
?>                
                </td>
...
<?php
    ...
    switch ($action) {
      ...
      if (isset($rInfo) && is_object($rInfo)) {
        ...
        $contents[] = array('text' => TEXT_INFO_REVIEW_RATING . ' ' . tep_image(DIR_WS_TEMPLATE_IMAGES . 'reviews/stars_' . $rInfo->reviews_rating . '.gif'));
