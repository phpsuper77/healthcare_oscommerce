<?php
/*
  $Id: affiliate_sales.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
*/
  ...
  if ($HTTP_GET_VARS['acID'] > 0) {
    ...
    $affiliate_sales_raw = "
      ...
      left join " . TABLE_ORDERS_STATUS . " os on (o.orders_status = os.orders_status_id and os.language_id = " . $languages_id . ") 
      left join " . TABLE_AFFILIATE . " a on (a.affiliate_id = asale.affiliate_id) 
      where asale.affiliate_id = '" . $HTTP_GET_VARS['acID'] . "' " . (tep_session_is_registered('login_affiliate')?" and asale.affiliate_id = '" . $login_id."'":''). " 
    ...
    $affiliate_sales_raw = "
      ...
      left join " . TABLE_ORDERS_STATUS . " os on (o.orders_status = os.orders_status_id and os.language_id = " . $languages_id . ") 
      left join " . TABLE_AFFILIATE . " a  on (a.affiliate_id = asale.affiliate_id) 
       " . (tep_session_is_registered('login_affiliate')?" where asale.affiliate_id = '" . $login_id."'":''). " 
      order by affiliate_date desc 
      ";
    $affiliate_sales_split = new splitPageResults($HTTP_GET_VARS['page'], MAX_DISPLAY_SEARCH_RESULTS, $affiliate_sales_raw, $affiliate_sales_numrows);
  }
  ...
  if ($affiliate_sales_numrows > 0) {
    ...
    while ($affiliate_sales = tep_db_fetch_array($affiliate_sales_values)) {
      ...
      if ($affiliate_sales['orders_status']) {
        ...
        $link_to = $affiliate_sales['affiliate_orders_id'];
      }
?>
