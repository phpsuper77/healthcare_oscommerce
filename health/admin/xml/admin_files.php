<?php
/*
  $Id: admin_files.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
<!-- header //-->
<?
$header_title_menu=BOX_HEADING_ADMINISTRATOR;
$header_title_menu_link=tep_href_link(FILENAME_ADMIN_ACCOUNT, tep_get_all_get_params(array('selected_box')) . 'selected_box=administrator');
...
?>
                ...
                <td class="dataTableContent" align="center"><?php
                                               if ( (is_object($cInfo)) && ($HTTP_GET_VARS['cID'] == $boxes[$i]['admin_boxes_id'])) {
                                                 if (substr($boxes[$i]['admin_boxes_id'], 0,1) == 'b') {
                                                   ...
                                                   echo ($HTTP_GET_VARS['cID'] != 1 ? '<a href="' . tep_href_link(FILENAME_ADMIN_FILES, 'cID=' . $HTTP_GET_VARS['cID'] . '&action=box_remove') . '">' : '') . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', STATUS_BOX_REMOVE, 10, 10) . ($HTTP_GET_VARS['cID'] != 1 ? '</a>' : '') . '&nbsp;' . tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', STATUS_BOX_INSTALLED, 10, 10);
                                                 }
                                               } else {
                                                 if (substr($boxes[$i]['admin_boxes_id'], 0,1) == 'b') {
                ...
                <td class="dataTableContent" align="right"><?php if ( (is_object($cInfo)) && ($boxes[$i]['admin_boxes_id'] == $cInfo->admin_boxes_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_ADMIN_FILES, 'cID=' . $boxes[$i]['admin_boxes_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
   $i++;
  }
