<?php
/*
  $Id: define_agb.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
*/
  ...
  $HTTP_GET_VARS['filename'] = 'agb.tpl.php';

  switch ($HTTP_GET_VARS['action']) {
    case 'save':
      if ( ($HTTP_GET_VARS['lngdir']) && ($HTTP_GET_VARS['filename']) ) {
        if (tep_session_is_registered('login_affiliate')){
          $file = DIR_FS_AFFILATES . $login_id . '/' . $HTTP_GET_VARS['lngdir'] . '/' . $HTTP_GET_VARS['filename'];
        }else{
        ...
        tep_redirect(tep_href_link(FILENAME_DEFINE_AGB, 'lngdir=' . $HTTP_GET_VARS['lngdir']));
      }
      break;
  }
...
<?
  ...
  $header_title_submenu=BOX_CATALOG_DEFINE_AGB;
  $header_title_additional=tep_draw_form('lng', FILENAME_DEFINE_AGB, '', 'get').tep_draw_pull_down_menu('lngdir', $languages_array, '', 'onChange="this.form.submit();"').'</form>';
?>
...
addEvent(window, 'load', initPage);
          ...
          <tr><?php echo tep_draw_form('language', FILENAME_DEFINE_AGB, 'lngdir=' . $HTTP_GET_VARS['lngdir'] . '&filename=' . $HTTP_GET_VARS['filename'] . '&action=save'); ?>
            <td><table border="0" cellspacing="0" cellpadding="2">
              ...
              <tr>
                <td align="right"><?php if ($file_writeable) { echo tep_image_submit('button_save.gif', IMAGE_SAVE) . '&nbsp;<a href="' . tep_href_link(FILENAME_DEFINE_AGB, 'lngdir=' . $HTTP_GET_VARS['lngdir']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; } else { echo '<a href="' . tep_href_link(FILENAME_DEFINE_AGB, 'lngdir=' . $HTTP_GET_VARS['lngdir']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>'; } ?></td>
              </tr>
            </table></td>
          </form></tr>
...
?>
          ...
          <tr>
            <td><?php echo '<a href="' . tep_href_link(FILENAME_DEFINE_AGB, 'lngdir=' . $HTTP_GET_VARS['lngdir']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
          </tr>
<?php
    }
  } else {
...
?>
          <tr>
            <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="smallText"><a href="<?php echo tep_href_link(FILENAME_DEFINE_AGB, 'lngdir=' . $HTTP_GET_VARS['lngdir'] . '&filename=' . $filename); ?>"><b><?php echo $filename; ?></b></a></td>
<?php
    ...
    if ($dir) {
      ...
      while ($file = $dir->read()) {
        if (substr($file, strrpos($file, '.')) == $file_extension) {
          echo '                <td class="smallText"><a href="' . tep_href_link(FILENAME_DEFINE_AGB, 'lngdir=' . $HTTP_GET_VARS['lngdir'] . '&filename=' . $file) . '">' . $file . '</a></td>' . "\n";
          if (!$left) {
            echo '              </tr>' . "\n" .
                 '              <tr>' . "\n";
          }
