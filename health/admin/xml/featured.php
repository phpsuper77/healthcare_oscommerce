...
*/
  ...
  switch ($HTTP_GET_VARS['action']) {
    ...
    case 'insert':
      ...
      tep_db_query("insert into " . TABLE_FEATURED . " (products_id, featured_date_added, expires_date, status, affiliate_id) values ('" . $HTTP_POST_VARS['products_id'] . "', now(), '" . $expires_date . "', '1', " . (tep_session_is_registered('login_affiliate')?$login_id:'0') . ")");
...
<!-- header //-->
<?
  if ( ($HTTP_GET_VARS['action'] == 'new') || ($HTTP_GET_VARS['action'] == 'edit') ) echo tep_init_calendar();
...
<!-- body_text //-->
    <td width="100%" valign="top" height="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0" height="100%" valign="top">
<?php
  if ( ($HTTP_GET_VARS['action'] == 'new') || ($HTTP_GET_VARS['action'] == 'edit') ) {
    $form_action = 'insert';
    if ( ($HTTP_GET_VARS['action'] == 'edit') && ($HTTP_GET_VARS['sID']) ) {
	  $form_action = 'update';
      $product_query = tep_db_query("select p.products_id, pd.products_name, s.expires_date from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_FEATURED . " s where p.products_id = pd.products_id and pd.language_id = '" . $languages_id . "' and p.products_id = s.products_id and s.featured_id = '" . $HTTP_GET_VARS['sID'] . "' and pd.affiliate_id = 0 " . (tep_session_is_registered('login_affiliate')?" and s.affiliate_id = '" . $login_id . "'":'') . " order by pd.products_name");
      ...
      <tr><form name="new_feature" <?php echo 'action="' . tep_href_link(FILENAME_FEATURED, tep_get_all_get_params(array('action', 'info', 'sID')) . 'action=' . $form_action, 'NONSSL') . '"'; ?> method="post"><?php if ($form_action == 'update') echo tep_draw_hidden_field('featured_id', $HTTP_GET_VARS['sID']); ?>
        <td valign=top><br><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            ...
            <td class="main"><?php echo tep_draw_calendar( 'new_feature', 'expires_date', $sInfo->expires_date ); ?></td>
          </tr>
        </table></td>
      </tr>
      <tr>
...
?>
              <td class="dataTableHeadingContent" align="right"><?php echo TEXT_AFFILIATE;?></td>
<?php
}
?>
...
<?php
    $featured_query_raw = "select p.products_id, pd.products_name, s.featured_id, s.featured_date_added, s.featured_last_modified, s.expires_date, s.date_status_change, s.status, s.affiliate_id from " . TABLE_PRODUCTS . " p, " . TABLE_FEATURED . " s, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and pd.language_id = '" . $languages_id . "' and p.products_id = s.products_id and pd.affiliate_id = 0 " . (tep_session_is_registered('login_affiliate')?" and s.affiliate_id = '" . $login_id . "'":'') . " order by pd.products_name";
...
if (!tep_session_is_registered('login_affiliate')){
?>
              <td class="dataTableContent" align="right"><?php echo tep_get_affiliate_name($featured['affiliate_id']);?></td>
<?php
}
?>
