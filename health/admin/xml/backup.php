<?php
/*
  $Id: backup.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
?>
                <td class="dataTableContent" onclick="document.location.href='<?php echo tep_href_link(FILENAME_BACKUP, $onclick_link); ?>'"><?php echo '<a href="' . tep_href_link(FILENAME_BACKUP, tep_session_name() . '=' . tep_session_id() . '&action=download&file=' . $entry) . '">' . tep_image(DIR_WS_ICONS . 'file_download.gif', ICON_FILE_DOWNLOAD) . '</a>&nbsp;' . $entry; ?></td>
...
<?php
  ...
  switch ($action) {
    case 'backup':
      ...
      $contents = array('form' => tep_draw_form('backup', FILENAME_BACKUP, 'action=backupnow&' . tep_session_name() . '=' . tep_session_id()));
      ...
      if (!$return_var) $contents[] = array('text' => '<br>' . tep_draw_radio_field('compress', 'gzip') . ' ' . TEXT_INFO_USE_GZIP);
      exec(LOCAL_EXE_ZIP, $output, $return_var);
      if (!$return_var) $contents[] = array('text' => tep_draw_radio_field('compress', 'zip') . ' ' . TEXT_INFO_USE_ZIP);
