<?php
/*
  $Id: banner_manager.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
*/
  ...
  if (isset($HTTP_GET_VARS['bID']) && tep_session_is_registered("login_affiliate")){
    $query  = tep_db_query("select * from " . TABLE_BANNERS . " where banners_id = '" . $bID. "' and affiliate_id = '" . $login_id. "'");
    ...
    switch ($action) {
      ...
      case 'update':
        ...
        if (tep_session_is_registered('login_affiliate')){
          $banners_group = tep_db_prepare_input($HTTP_POST_VARS['banners_group']);
        }else{
        ...
        $db_image_location = '';
        $expires_date = tep_calendar_rawdate($HTTP_POST_VARS['expires_date']);
        $expires_impressions = tep_db_prepare_input($HTTP_POST_VARS['expires_impressions']);
        $date_scheduled = tep_calendar_rawdate($HTTP_POST_VARS['date_scheduled']);
        ...
        if (empty($banners_html_text)) {
          if (empty($banners_image_local)) {
            ...
            if (tep_session_is_registered('login_affiliate')){
              if (!is_dir(DIR_FS_CATALOG_IMAGES . 'banners/' . $login_id)){
                @mkdir(DIR_FS_CATALOG_IMAGES . 'banners/' . $login_id);
                @chmod(DIR_FS_CATALOG_IMAGES . 'banners/' . $login_id, 0777);
              }
              $banners_image->set_destination(DIR_FS_CATALOG_IMAGES . 'banners/' . $login_id);
              $banners_image_target = 'banners/' . $login_id . '/' . $banners_image_target;
            }else{
...
<?php
  if ($action == 'new') {
    ...
    if (isset($HTTP_GET_VARS['bID'])) {
      ...
      $banner_query = tep_db_query("select banners_title, banners_url, banners_image, banners_group, banners_html_text, status, date_scheduled, expires_date, expires_impressions, date_status_change from " . TABLE_BANNERS . " where banners_id = '" . (int)$bID . "'");
      $banner = tep_db_fetch_array($banner_query);
...
?>
      ...
      <tr><?php echo tep_draw_form('new_banner', FILENAME_BANNER_MANAGER, (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . 'action=' . $form_action, 'post', 'enctype="multipart/form-data"'); if ($form_action == 'update') echo tep_draw_hidden_field('banners_id', $bID); ?>
        <td height="100%"><table border="0" cellspacing="0" cellpadding="2" height="100%">
          <tr>
          ...
          <tr>
            ...
            <td class="main"><?php echo tep_draw_pull_down_menu('banners_group', $groups_array, $bInfo->banners_group) . (!tep_session_is_registered('login_affiliate')?TEXT_BANNERS_NEW_GROUP . '<br>' . tep_draw_input_field('new_banners_group', '', '', ((sizeof($groups_array) > 0) ? false : true)):''); ?></td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td class="main"><?php echo tep_draw_file_field('banners_image') . (!tep_session_is_registered('login_affiliate')?' ' . TEXT_BANNERS_IMAGE_LOCAL . '<br>' . DIR_FS_CATALOG_IMAGES . tep_draw_input_field('banners_image_local', (isset($bInfo->banners_image) ? $bInfo->banners_image : '')):''); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
          <tr>
            ...
            <td class="main"><?php echo DIR_FS_CATALOG_IMAGES .(tep_session_is_registered('login_affiliate')?'banners/' . $login_id . '/':'') .  tep_draw_input_field('banners_image_target'); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
          ...
          <tr>
            <td class="main"><?php echo TEXT_BANNERS_SCHEDULED_AT; ?><br><small>(<?php echo strtolower(DATE_FORMAT_SPIFFYCAL); ?>)</small></td>
            <td valign="top" class="main"><?php echo tep_draw_calendar('new_banner','date_scheduled', $bInfo->date_scheduled ); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
          <tr>
            <td valign="top" class="main"><?php echo TEXT_BANNERS_EXPIRES_ON; ?><br><small>(<?php echo strtolower(DATE_FORMAT_SPIFFYCAL); ?>)</small></td>
            <td class="main"><?php echo tep_draw_calendar('new_banner','expires_date', $bInfo->expires_date ); ?><?php echo TEXT_BANNERS_OR_AT . '<br>' . tep_draw_input_field('expires_impressions', $bInfo->expires_impressions, 'maxlength="7" size="7"') . ' ' . TEXT_BANNERS_IMPRESSIONS; ?></td>
          </tr>
        </table></td>
      </tr>
      <tr>
...
if (!tep_session_is_registered("login_affiliate")){
?>                
                <td class="dataTableHeadingContent" align="right"><?php echo TABLE_HEADING_AFFILIATE; ?></td>
<?php
}
?>
...
<?php
    $banners_query_raw = "select banners_id, banners_title, banners_image, banners_group, status, expires_date, expires_impressions, date_status_change, date_scheduled, date_added, affiliate_id from " . TABLE_BANNERS . (tep_session_is_registered('login_affiliate')?" where affiliate_id = '" . $login_id . "'":'') .  " order by banners_title, banners_group";
...
?>                
                ...
                if ($banners['affiliate_id'] > 0){
                  $data = tep_db_fetch_array(tep_db_query("select * from " . TABLE_AFFILIATE . " where affiliate_id = '" .$banners['affiliate_id']. "'"));
                  echo '<a href="' . tep_href_link(FILENAME_AFFILIATE, 'acID=' . $banners['affiliate_id'] . '&action=edit') . '">' . tep_get_affiliate_name($banners['affiliate_id']) . '</a>';
                }else{
                  echo DATA_ADMIN; 
                }
                ?></td>
