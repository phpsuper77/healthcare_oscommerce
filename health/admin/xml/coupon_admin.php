<?php
/*
  $Id: coupon_admin.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
*/
  ...
  if (($HTTP_GET_VARS['action'] == 'send_email_to_user') && ($HTTP_POST_VARS['customers_email_address']) && (!$HTTP_POST_VARS['back_x'])) {
    ...
    $coupon_name_query = tep_db_query("select coupon_name, coupon_description from " . TABLE_COUPONS_DESCRIPTION . " where coupon_id = '" . $HTTP_GET_VARS['cid'] . "' and language_id = '" . $languages_id . "'");
    ...
    while ($mail = tep_db_fetch_array($mail_query)) {
      ...
      $message .= $coupon_name['coupon_description'] . "\n\n";
      $message .= "\n\n" . TEXT_TO_REDEEM . "\n\n";
  ...
  switch ($HTTP_GET_VARS['action']) {
    ...
    case 'update':
      ...
      $HTTP_POST_VARS['coupon_startdate'] = tep_calendar_rawdate($HTTP_POST_VARS['coupon_startdate']);
      $HTTP_POST_VARS['coupon_finishdate'] = tep_calendar_rawdate($HTTP_POST_VARS['coupon_finishdate']);
...
<?
  ...
  if($HTTP_GET_VARS['action']!='voucherreport'&& $HTTP_GET_VARS['action']!='preview_email' && $HTTP_GET_VARS['action']!='email' && $HTTP_GET_VARS['action']!='update_preview' && $HTTP_GET_VARS['action']!='voucheredit'    && $HTTP_GET_VARS['action']!='new')
  {
   $header_title_additional = tep_draw_form('status', FILENAME_COUPON_ADMIN, '', 'get');
   ...
   $header_title_additional .=HEADING_TITLE_STATUS . ' ' . tep_draw_pull_down_menu('status', $status_array, $status, 'onChange="this.form.submit();"').'</form>';
  }
?>
...
// bug 2007-11-06    $cc_split = new splitPageResults($HTTP_GET_VARS['page'], MAX_DISPLAY_SEARCH_RESULTS, $cc_query_raw, $cc_query_numrows);
...
<?php
    ...
    $contents = array();
      ...
      $contents[] = array('text' => '');
?>
    <td width="25%" valign="top" background="images/right_bg.gif">
<?php
      $box = new box;
...
?>
      <tr>
        ...
        <td align="left" class="main"><?php echo $HTTP_POST_VARS['coupon_name'][$language_id]; ?></td>
      </tr>
<?php
}
?>
...
?>
      <tr>
        ...
        <td align="left" class="main"><?php echo $HTTP_POST_VARS['coupon_desc'][$language_id]; ?></td>
      </tr>
<?php
}
?>
      <tr>
        ...
        <td align="left" class="main"><?php echo $HTTP_POST_VARS['coupon_amount']; ?></td>
      </tr>

      <tr>
        <td align="left" class="main"><?php echo COUPON_MIN_ORDER; ?></td>
        <td align="left" class="main"><?php echo $HTTP_POST_VARS['coupon_min_order']; ?></td>
      </tr>
...
?>
      ...
      <tr>
        ...
        <td align="left" class="main"><?php echo $HTTP_POST_VARS['coupon_uses_coupon']; ?></td>
      </tr>

      <tr>
        <td align="left" class="main"><?php echo COUPON_USES_USER; ?></td>
        <td align="left" class="main"><?php echo $HTTP_POST_VARS['coupon_uses_user']; ?></td>
      </tr>

       <tr>
        <td align="left" class="main"><?php echo COUPON_PRODUCTS; ?></td>
        <td align="left" class="main"><?php echo $HTTP_POST_VARS['coupon_products']; ?></td>
      </tr>


      <tr>
        <td align="left" class="main"><?php echo COUPON_CATEGORIES; ?></td>
        <td align="left" class="main"><?php echo $HTTP_POST_VARS['coupon_categories']; ?></td>
      </tr>
      <tr>
        <td align="left" class="main"><?php echo COUPON_STARTDATE; ?></td>
<?php
      $start_date = tep_date_short($HTTP_POST_VARS['coupon_startdate']);
?>
...
<?php
    $finish_date = tep_date_short($HTTP_POST_VARS['coupon_finishdate']);
?>
...
?>
      ...
      <tr>
<?php
    if ( isset($HTTP_POST_VARS['coupon_startdate']) ) {
      $coupon_start_date = $HTTP_POST_VARS['coupon_startdate'];
    }elseif(isset($coupon['coupon_start_date'])){
      $coupon_start_date = $coupon['coupon_start_date'];
    }else{
      $coupon_start_date = date("Ymd");
    }
    if ( isset($HTTP_POST_VARS['coupon_finishdate']) ) {
      $coupon_finish_date = $HTTP_POST_VARS['coupon_finishdate'];
    }elseif(isset($coupon['coupon_expire_date'])){
      $coupon_finish_date = $coupon['coupon_expire_date'];
    }else{
      $coupon_finish_date = date("Ymd",mktime(0, 0, 0, date("m"),date("d"),date("Y")+1));
    }
?>
        <td align="left" class="main"><?php echo COUPON_STARTDATE; ?></td>
        <td align="left"><?php echo tep_draw_calendar('coupon','coupon_startdate',$coupon_start_date,true); ?></td>
        <td align="left" class="main"><?php echo COUPON_STARTDATE_HELP; ?></td>
      </tr>
      <tr>
        <td align="left" class="main"><?php echo COUPON_FINISHDATE; ?></td>
        <td align="left"><?php echo tep_draw_calendar('coupon','coupon_finishdate',$coupon_finish_date,true); ?></td>
    ...
    default:
      ...
      if ($HTTP_GET_VARS['action'] == 'voucherdelete') {
        $contents[] = array('text'=> TEXT_CONFIRM_DELETE . '</br></br>' .
                '<a href="'.tep_href_link('coupon_admin.php','action=confirmdelete&cid='.$HTTP_GET_VARS['cid'],'NONSSL').'">'.tep_image_button('button_confirm.gif','Confirm Delete Voucher').'</a>' .
        ...
        $contents[] = array('text'=>COUPON_NAME . '&nbsp;::&nbsp; ' . $coupon_name['coupon_name'] . '<br>' .
                     ...
                     '<center><a href="'.tep_href_link('coupon_admin.php','action=email&cid='.$cInfo->coupon_id,'NONSSL').'">'.tep_image_button('button_email.gif',TEXT_EMAIL_COUPON).'</a>' .
                     '<a href="'.tep_href_link('coupon_admin.php','action=voucheredit&cid='.$cInfo->coupon_id,'NONSSL').'">'.tep_image_button('button_edit.gif', TEXT_EDIT_COUPON).'</a>' .
                     '<a href="'.tep_href_link('coupon_admin.php','action=voucherdelete&cid='.$cInfo->coupon_id,'NONSSL').'">'.tep_image_button('button_delete.gif',TEXT_DELETE_COUPON).'</a>' .
                     '<br><a href="'.tep_href_link('coupon_admin.php','action=voucherreport&cid='.$cInfo->coupon_id,'NONSSL').'">'.tep_image_button('button_report.gif',TEXT_COUPON_REPORT).'</a></center>'
                     );
        }
        break;
      }
