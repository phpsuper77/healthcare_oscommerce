<?php
/*
  $Id: define_vendor_terms.php,v 1.1.1.1 2005/12/03 21:36:02 max Exp $
...
// This will cause it to look for 'mainpage.php'
  ...
  $HTTP_GET_VARS['filename'] = 'vendor_terms.tpl.php';
  ...
  switch ($HTTP_GET_VARS['action']) {
    case 'save':
      if ( ($HTTP_GET_VARS['lngdir']) && ($HTTP_GET_VARS['filename']) ) {
        ...
        tep_redirect(tep_href_link(FILENAME_DEFINE_VENDOR_TERMS, 'lngdir=' . $HTTP_GET_VARS['lngdir']));
      }
      break;
  }
...
<?
  ...
  $header_title_menu_link= tep_href_link(FILENAME_VENDOR, 'selected_box=vendor');
  $header_title_submenu=BOX_VENDOR_DEFINE_VENDOR_TERMS;
  $header_title_additional=tep_draw_form('lng', FILENAME_DEFINE_VENDOR_TERMS, '', 'get').tep_draw_pull_down_menu('lngdir', $languages_array, '', 'onChange="this.form.submit();"').'</form>';
?>
...
addEvent(window, 'load', initPage);
          ...
          <tr><?php echo tep_draw_form('language', FILENAME_DEFINE_VENDOR_TERMS, 'lngdir=' . $HTTP_GET_VARS['lngdir'] .  '&action=save'); ?>
            <td><table border="0" cellspacing="0" cellpadding="2">
              <tr>
                <td class="main"><b><?php echo $HTTP_GET_VARS['filename']; ?></b></td>
              </tr>
              <tr>
                <td class="main"><?php echo tep_draw_textarea_field('file_contents', 'soft', '80', '20', $file_contents, 'id="editor" class="editor" style="width: 100%;" ' . (($file_writeable) ? '' : 'readonly')); ?></td>
              </tr>
              <tr>
                <td><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
              </tr>
              <tr>
                <td align="right"><?php if ($file_writeable) { echo tep_image_submit('button_save.gif', IMAGE_SAVE) . '&nbsp;<a href="' . tep_href_link(FILENAME_DEFINE_VENDOR_TERMS, 'lngdir=' . $HTTP_GET_VARS['lngdir']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; } else { echo '<a href="' . tep_href_link(FILENAME_DEFINE_VENDOR_TERMS, 'lngdir=' . $HTTP_GET_VARS['lngdir']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>'; } ?></td>
              </tr>
            </table></td>
          </form></tr>
...
?>
          ...
          <tr>
            <td><?php echo '<a href="' . tep_href_link(FILENAME_DEFINE_VENDOR_TERMS, 'lngdir=' . $HTTP_GET_VARS['lngdir']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
          </tr>
<?php
    }
  } else {
...
?>
          <tr>
            <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="smallText"><a href="<?php echo tep_href_link(FILENAME_DEFINE_VENDOR_TERMS, 'lngdir=' . $HTTP_GET_VARS['lngdir'] . '&filename=' . $filename); ?>"><b><?php echo $filename; ?></b></a></td>
<?php
    ...
    if ($dir) {
      ...
      while ($file = $dir->read()) {
        if (substr($file, strrpos($file, '.')) == $file_extension) {
          echo '                <td class="smallText"><a href="' . tep_href_link(FILENAME_DEFINE_VENDOR_TERMS, 'lngdir=' . $HTTP_GET_VARS['lngdir']) . '">' . $file . '</a></td>' . "\n";
          if (!$left) {
            echo '              </tr>' . "\n" .
                 '              <tr>' . "\n";
          }
