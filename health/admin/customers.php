...
*/
  ...
  if (tep_not_null($action)) {
    switch ($action) {
 case 'get_customers':
            $sub_query = tep_db_query("select customers_firstname as firstname,customers_lastname as lastname, customers_email_address as email, if (customers_newsletter=1,'Subscribed','Unsubscribed') as newsletter from " . TABLE_CUSTOMERS . "  group by customers_email_address order by firstname");
            if (tep_db_num_rows($sub_query) > 0) {
                $fp = fopen(DIR_FS_CATALOG . 'temp/' . 'customers.csv', 'w');
                tep_fputcsv($fp, array('First name','Surname', 'Email', 'Subscribe'));
                while ($nsData = tep_db_fetch_array($sub_query))
                    tep_fputcsv($fp, $nsData);
            }
            ...
            readfile(DIR_FS_CATALOG . 'temp/' . 'customers.csv');
            exit();
            break;

        case 'get_subscribers':

            $sub_query = tep_db_query("select subscribers_firstname as firstname, subscribers_lastname as lastname, subscribers_email_address  as email  from " . TABLE_SUBSCRIBERS . "  group by subscribers_email_address order by firstname");
            if (tep_db_num_rows($sub_query) > 0) {
                $fp = fopen(DIR_FS_CATALOG . 'temp/' . 'subscribers.csv', 'w');
                tep_fputcsv($fp, array('First name','Surname', 'Email'));
                while ($nsData = tep_db_fetch_array($sub_query)) {
                    tep_fputcsv($fp, $nsData);
                }
                header('Content-Type: text/comma-separated-values');
                ...
                readfile(DIR_FS_CATALOG . 'temp/' . 'subscribers.csv');
            }
            exit();
    ...
    default:
      if (isset($cInfo) && is_object($cInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> ' . (!tep_session_is_registered('login_affiliate')?' <a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=confirm') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>':'') . ' <a href="' . tep_href_link(FILENAME_ORDERS, 'cID=' . $cInfo->customers_id) . '">' . tep_image_button('button_orders.gif', IMAGE_ORDERS) . '</a> <a href="' . tep_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $cInfo->customers_email_address) . '">' . tep_image_button('button_email.gif', IMAGE_EMAIL) . '</a>'.
            '<a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('action')) . '&action=get_customers') . '">' . tep_image_button('button_customer_newsletter.gif', IMAGE_CUSTOMER_NEWSLETTER) . '</a>'.
            '<a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('action')) . '&action=get_subscribers') . '">' . tep_image_button('button_customer_subscribers.gif', IMAGE_CUSTOMER_SUBSCRIBERS) . '</a>');
