...
<?
  ...
  if (!(count($custid) > 0) )
  {
    $header_title_additional='<form method=post action='.$PHP_SELF.'><table align="right" width="100%"><tr class="dataTableContent" align="right"><td>'.DAYS_FIELD_PREFIX.'<input type=text size=4 width=4 value='.$sdate.' name=sdate> - <input type=text size=4 width=4 value='.$tdate.' name=tdate>'.DAYS_FIELD_POSTFIX.'<input type=submit value="'.DAYS_FIELD_BUTTON.'"></td></tr></table></form>';
  }
  else
  {
    $header_title_submenu.=" (".HEADING_EMAIL_SENT.")";
  }
?>
