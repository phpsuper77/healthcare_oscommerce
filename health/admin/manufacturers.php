<?php
/*
  $Id: manufacturers.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'save':
        ...
        if (isset($HTTP_POST_VARS['remove_image']) && ($HTTP_POST_VARS['remove_image'] == 'on')) {
          $sql_data_array['manufacturers_image'] = '';
        }
        
        if ( (int)$manufacturers_id!=0 && isset($HTTP_POST_VARS['delete_image']) && ($HTTP_POST_VARS['delete_image'] == 'on')) {
          $sql_data_array['manufacturers_image'] = '';
        ...
        if (tep_not_null($manufacturers_image->filename)) {
...
<?php
  ...
  switch ($action) {
    case 'new':
      ...
      $manufacturer_inputs_string = ''; $manuf_meta_desc = ''; $manuf_meta_key = ''; $manuf_meta_title = ''; $manuf_seo_name = '';
      $languages = tep_get_languages();
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        ...
        $manuf_meta_desc .= '<br><table border="0" cellspacing="0" cellpadding="0">
				 <tr valign="top">
				  <td>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '</td>
				  <td>&nbsp;</td>
				  <td>' . tep_draw_textarea_field('manufacturers_meta_description[' . $languages[$i]['id'] . ']', 'soft', '25', '7') . '</td>
				</tr>
			</table>';
        $manuf_meta_key .= '<br><table border="0" cellspacing="0" cellpadding="0">
				 <tr valign="top">
				  <td>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '</td>
				  <td>&nbsp;</td>
				  <td>' . tep_draw_textarea_field('manufacturers_meta_key[' . $languages[$i]['id'] . ']', 'soft', '25', '7') . '</td>
				</tr>
			</table>';
			 $manuf_meta_title .= '<br><table border="0" cellspacing="0" cellpadding="0">
				 <tr valign="top">
				  <td>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '</td>
				  <td>&nbsp;</td>
				  <td>' . tep_draw_input_field('manufacturers_meta_title[' . $languages[$i]['id'] . ']') . '</td>
				</tr>
			</table>';
			$manuf_seo_name .= '<br><table border="0" cellspacing="0" cellpadding="0">
				 <tr valign="top">
				  <td>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '</td>
				  <td>&nbsp;</td>
				  <td>' . tep_draw_input_field('manufacturers_seo_name[' . $languages[$i]['id'] . ']') . '</td>
				</tr>
			</table>';
      }

      $contents[] = array('text' => '<br>' . TEXT_MANUFACTURERS_URL . $manufacturer_inputs_string);
      $contents[] = array('text' => '<br>' . TEXT_MANUFACTURERS_META_TITLE . $manuf_meta_title);
      $contents[] = array('text' => '<br>' . TEXT_MANUFACTURERS_META_DESCRIPTION . $manuf_meta_desc);
      $contents[] = array('text' => '<br>' . TEXT_MANUFACTURERS_META_KEYWORDS . $manuf_meta_key);
      $contents[] = array('text' => '<br>' . TEXT_MANUFACTURERS_SEO_NAME . $manuf_seo_name);
    ...
    case 'edit':
      ...
      $contents[] = array('text' => '' . tep_draw_checkbox_field('remove_image', '', false) . ' ' . TEXT_REMOVE_IMAGE);
      $contents[] = array('text' => '' . tep_draw_checkbox_field('delete_image', '', false) . ' ' . TEXT_DELETE_IMAGE);
      ...
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        ...
        $manuf_meta_desc .= '<br><table border="0" cellspacing="0" cellpadding="0">
				 <tr valign="top">
				  <td>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '</td>
				  <td>&nbsp;</td>
				  <td>' . tep_draw_textarea_field('manufacturers_meta_description[' . $languages[$i]['id'] . ']', 'soft', '25', '7', tep_get_manufacturer_meta_descr($mInfo->manufacturers_id, $languages[$i]['id'])) . '</td>
				</tr>
			</table>';
        $manuf_meta_key .= '<br><table border="0" cellspacing="0" cellpadding="0">
				 <tr valign="top">
				  <td>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '</td>
				  <td>&nbsp;</td>
				  <td>' . tep_draw_textarea_field('manufacturers_meta_key[' . $languages[$i]['id'] . ']', 'soft', '25', '7', tep_get_manufacturer_meta_key($mInfo->manufacturers_id, $languages[$i]['id'])) . '</td>
				</tr>
			</table>';
			$manuf_meta_title .= '<br><table border="0" cellspacing="0" cellpadding="0">
				 <tr valign="top">
				  <td>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '</td>
				  <td>&nbsp;</td>
				  <td>' . tep_draw_input_field('manufacturers_meta_title[' . $languages[$i]['id'] . ']', tep_get_manufacturer_meta_title($mInfo->manufacturers_id, $languages[$i]['id'])) . '</td>
				</tr>
			</table>';
			$manuf_seo_name .= '<br><table border="0" cellspacing="0" cellpadding="0">
				 <tr valign="top">
				  <td>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '</td>
				  <td>&nbsp;</td>
				  <td>' . tep_draw_input_field('manufacturers_seo_name[' . $languages[$i]['id'] . ']', tep_get_manufacturer_seo_name($mInfo->manufacturers_id, $languages[$i]['id'])) . '</td>
				</tr>
			</table>';
      }
