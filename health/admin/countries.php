<?php
/*
  $Id: countries.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
//        $countries_name = tep_db_prepare_input($HTTP_POST_VARS['countries_name']);
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
          ...
          if ($i == 0){
            tep_db_query("insert into " . TABLE_COUNTRIES . " (countries_name, countries_iso_code_2, countries_iso_code_3, address_format_id, language_id) values ('" . tep_db_input($countries_name) . "', '" . tep_db_input($countries_iso_code_2) . "', '" . tep_db_input($countries_iso_code_3) . "', '" . (int)$address_format_id . "', '".$language_id."')");
            $id = tep_db_insert_id();
          }else{
            tep_db_query("insert into " . TABLE_COUNTRIES . " (countries_id, countries_name, countries_iso_code_2, countries_iso_code_3, address_format_id, language_id) values (".$id.", '" . tep_db_input($countries_name) . "', '" . tep_db_input($countries_iso_code_2) . "', '" . tep_db_input($countries_iso_code_3) . "', '" . (int)$address_format_id . "', '".$language_id."')");
          }
        }
...
//        $countries_name = tep_db_prepare_input($HTTP_POST_VARS['countries_name']);
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
          ...
          $countries_name = tep_db_prepare_input($countries_name_array[$language_id]);
          tep_db_query("update " . TABLE_COUNTRIES . " set countries_name = '" . tep_db_input($countries_name) . "', countries_iso_code_2 = '" . tep_db_input($countries_iso_code_2) . "', countries_iso_code_3 = '" . tep_db_input($countries_iso_code_3) . "', address_format_id = '" . (int)$address_format_id . "' where countries_id = '" . (int)$countries_id . "' and language_id='".$language_id."'");
        }
...
<!-- header //-->
<?
  $header_title_menu=BOX_HEADING_LOCATION_AND_TAXES;
  $header_title_menu_link= tep_href_link(FILENAME_COUNTRIES, 'selected_box=taxes');
...
<?php
  $countries_query_raw = "select countries_id, countries_name, countries_iso_code_2, countries_iso_code_3, address_format_id from " . TABLE_COUNTRIES . " where language_id = '" . (int)$languages_id . "' order by countries_name";
...
for ($i=0; $i<sizeof($languages); $i++) {
  if ($i == 0) $text .= TEXT_INFO_COUNTRY_NAME;
  $text .= '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('countries_name[' . $languages[$i]['id'] . ']');
    }
...
for ($i=0; $i<sizeof($languages); $i++) {
  if ($i == 0) $text .= TEXT_INFO_COUNTRY_NAME;
  $text .= '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('countries_name[' . $languages[$i]['id'] . ']', tep_get_country_name($cInfo->countries_id, $languages[$i]['id']));
    }
      $contents[] = array('text' => $text);
