 ...
 * @author $Author: max $
 * @version $Id: ImageManager.php,v 1.1.1.1 2005/12/03 21:36:04 max Exp $
...
class ImageManager
{
  ...
  function getFiles($path)
  {
    ...
    $restrictImagesRead = $this->config['max_image_count'] == -1 ? false : true;
    ...
    while (false !== ($entry = $d->read()))
    {
      ...
      if(substr($entry,0,1) != '.') {
        ...
        if(is_dir($fullpath.$entry) && $this->isThumbDir($entry) == false) {
          ...
          if( ( ($restrictImagesRead == true && $countImages < $this->config['max_image_count']) || $restrictImagesRead == false) && is_file($fullpath.$entry) && $this->isThumb($entry)==false && $this->isTmpFile($entry) == false) {
