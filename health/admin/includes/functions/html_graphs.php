<?php
/*
  $Id: html_graphs.php,v 1.1.1.1 2005/12/03 21:36:03 max Exp $
...
// draws a double vertical bar graph for the banner views vs clicks statistics
  function tep_banner_graph_monthly($banner_id) {
    ...
    for ($i=1; $i<13; $i++) {
      $names[] = strftime('%b', mktime(0,0,0,$i,1));
      $values[] = '0';
      $dvalues[] = '0';
    }
    ...
    while ($banner_stats = tep_db_fetch_array($banner_stats_query)) {
      $names[($banner_stats['banner_month']-1)] = strftime('%b', mktime(0,0,0,$banner_stats['banner_month'],1));
...
// draws a double vertical bar graph for the banner views vs clicks statistics
  function tep_banner_graph_daily($banner_id) {
    ...
    $graph_vals = @array('vlabel'=>TEXT_BANNERS_DATA,
                        'hlabel'=>sprintf(TEXT_BANNERS_DAILY_STATISTICS, $banner['banners_title'], strftime('%B', mktime(0,0,0,$month,1)), $year),
                        'type'=>'3',
                        'cellpadding'=>'',
                        'cellspacing'=>'1',
                        'border'=>'',
