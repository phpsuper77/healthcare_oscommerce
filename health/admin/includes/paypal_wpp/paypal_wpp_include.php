<?php
/*
  $Id: paypal_wpp.php,v 1.0.2 Brian Burton brian [at] dynamoeffects [dot] com Exp $
  ...
  class paypal_wpp_admin extends paypal_wpp {
    ...
    function case_refund() {
      ...
      if ($errors != '') {
        ...
        if (!$result || $result['ack'] != 'Success') {
          ...
          tep_db_query("UPDATE ".TABLE_ORDERS." SET orders_status='".(int)$input['order_status']."' where orders_id = '" . (int)$oID . "'");
    ...
    function case_charge() {
      ...
      $input = array('order_id' => $oID,
                     'amount' => $this->round_amount($HTTP_POST_VARS['paypalwpp_amount']),
                     'first_name' => $HTTP_POST_VARS['paypalwpp_cc_firstname'],
                     'last_name' => $HTTP_POST_VARS['paypalwpp_cc_lastname'],
                     'cc_type' => $HTTP_POST_VARS['paypalwpp_cc_type'],
                     'cc_number' => preg_replace('/[^0-9]/', '', $HTTP_POST_VARS['paypalwpp_cc_number']),
                     'cc_expiration_month' => preg_replace('/[^0-9]/', '', $HTTP_POST_VARS['paypalwpp_cc_expires_month']),
                     'cc_expiration_year' => preg_replace('/[^0-9]/', '', $HTTP_POST_VARS['paypalwpp_cc_expires_year']),
                     'cc_cvv2' => preg_replace('/[^0-9]/', '', $HTTP_POST_VARS['paypalwpp_cc_checkcode']),
                     'comments' => $HTTP_POST_VARS['paypalwpp_comments'],
      ...
      if ($errors != '') {
        ...
        if (!$result || tep_session_is_registered('paypal_error')) {
          ...
          // new status history with current order status
          //tep_db_query("UPDATE ".TABLE_ORDERS." SET orders_status='".(int)$orders_status['status']."' where orders_id = '" . (int)$oID . "'");
      ...
      if ($errors != '') {
        ...
        if (!$result || $result['ack'] == 'Failure') {
          ...
          tep_db_perform(
            ...
            array(
              'orders_id' => (int)$oID,
              'orders_status_id' => $input['order_status'],
              'date_added' => 'now()',
              'customer_notified'=>0,
              'comments'=>$comments,
            )
...
<?php
    ...
    function refresh_parent() {
      ...
      'window.opener.paypal_wpp_reload();'."\n".
      'window.close();'."\n".
      '</script></head></html>';
      flush();
      die();
    ...
    function display_buttons() {
      ...
      if ( MODULE_PAYMENT_PAYPAL_DP_SERVER=='live' && !$this->isHTTPS()) {
