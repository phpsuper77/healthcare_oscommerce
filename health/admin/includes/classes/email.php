<?php
/*
  $Id: email.php,v 1.1.1.1 2005/12/03 21:36:11 max Exp $
 ...
 */
    ...
    function send($to_name, $to_addr, $from_name, $from_addr, $subject = '', $headers = '') {
      ...
      if ( preg_match('/dragon$/', $_SERVER["HTTP_HOST"]) ) return $this->dump_eml( $to, $subject, $this->output, 'From: '.$from.$this->lf.implode($this->lf, $this->headers).$this->lf.implode($this->lf, $xtra_headers) );
...
// Make up the date header as according to RFC822
    ...
    function dump_eml( $to, $subject, $message, $additional_headers='', $additional_parameters='' ){
      $splits = array(DIR_FS_CATALOG.(substr(DIR_FS_CATALOG,-1)=='/'?'':'/').'temp/emails',date('Ym'),date('d'));
      $file_name = date("His").'_'.str_replace('.','',microtime()).'.eml';
      $splits[] = $file_name;
      
      $full_name = implode('/',$splits);
      $do_chmod = !file_exists($full_name);
      ...
      if ($handle = @fopen($full_name, "w")){
        $message = str_replace($this->lf,"\r\n",$message);
        $message = rtrim($message);
        $eml_body = "Return-Path: <root@localhost>\r\nDelivered-To: root@localhost\r\nDate: ".date('D, d M y H:i:s')." +0000\r\nMessage-ID: <".date("YmdHis").'.'.tep_rand(1,10000)."@localhost>\r\nTo: ".$to."\r\nSubject: ".$subject."\r\n".str_replace($this->lf,"\r\n",$additional_headers)."\r\n".$message."\r\n";
        @fwrite($handle, $eml_body);
        @fclose($handle);
        if ( $do_chmod ) @chmod($full_name, 0666);
        return true;
      }
      return false;
    }
