<?php
/*
  $Id: affiliate_payment.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
...
// Get affiliate Informations
        ...
        SELECT a.*, c.countries_id, c.countries_name, c.countries_iso_code_2, c.countries_iso_code_3, c.address_format_id 
          ...
          WHERE affiliate_id = '" . $affiliate_payment['affiliate_id'] . "' and c.language_id = '" . (int)$languages_id . "' 
        ";
...
<?
  ...
  $header_title_additional=tep_draw_form('orders', FILENAME_AFFILIATE_PAYMENT, '', 'get').HEADING_TITLE_SEARCH . ' ' . tep_draw_input_field('sID', '', 'size="12"') . tep_draw_hidden_field('action', 'edit').'</form><br>';
  $header_title_additional.=tep_draw_form('status', FILENAME_AFFILIATE_PAYMENT, '', 'get').HEADING_TITLE_STATUS . ' ' . tep_draw_pull_down_menu('status', array_merge(array(array('id' => '', 'text' => TEXT_ALL_PAYMENTS)), $payments_statuses), '', 'onChange="this.form.submit();"').'</form>';
?>
<?php
   ...
   if (is_numeric($HTTP_GET_VARS['status'])) {
     $status = tep_db_prepare_input($HTTP_GET_VARS['status']);
      ...
      select p.* , s.affiliate_payment_status_name 
             ...
             and s.affiliate_payment_status_id = '" . tep_db_input($status) . "' 
             and s.affiliate_language_id = '" . $languages_id . "' 
             and p.affiliate_id =  '" . $login_id . "' 
    ...
    $affiliate_payment_split = new splitPageResults($HTTP_GET_VARS['page'], MAX_DISPLAY_SEARCH_RESULTS, $affiliate_payment_raw, $affiliate_payment_numrows);
?>
      <tr>
...
//$products_split->display_count($products_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['page'], TEXT_DISPLAY_NUMBER_OF_PRODUCTS);
    ...
    $affiliate_payment_values = tep_db_query($affiliate_payment_raw );
  if (tep_db_num_rows($affiliate_payment_values) > 0) {
    $number_of_payment = 0;
?>
...
?>
              <tr>
                <td colspan="6"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $affiliate_payment_split->display_count($affiliate_payment_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['page'], TEXT_DISPLAY_NUMBER_OF_PAYMENTS); ?></td>
                    <td class="smallText" align="right"><?php echo $affiliate_payment_split->display_links($affiliate_payment_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $HTTP_GET_VARS['page'], tep_get_all_get_params(array('page', 'pID', 'action'))); ?></td>
                  </tr>
                </table></td>
              </tr>

<?php 
  $affiliate_payment_values = tep_db_query("select sum(affiliate_payment_total) as total from " . TABLE_AFFILIATE_PAYMENT . " where affiliate_id = '" . $login_id . "'");
  $affiliate_payment = tep_db_fetch_array($affiliate_payment_values);
?>
          <tr>
            <td class="smallText" colspan="2"><?php echo TEXT_AFFILIATE_HEADER . ' ' . tep_db_num_rows(tep_db_query($affiliate_payment_raw)); ?></td>
            <td class="smalltext" colspan="2" align="center"><b><?php echo $currencies->display_price($affiliate_payment['total'], ''); ?></b></td>
          </tr>
        </table></td>
      </tr>
<?php
...
<?php
    if ($HTTP_GET_VARS['sID']) {
      ...
      $payments_query_raw = "select p.* , s.affiliate_payment_status_name from " . TABLE_AFFILIATE_PAYMENT . " p , " . TABLE_AFFILIATE_PAYMENT_STATUS . " s where p.affiliate_payment_id = '" . tep_db_input($sID) . "' and p.affiliate_payment_status = s.affiliate_payment_status_id and s.affiliate_language_id = '" . $languages_id . "' " . ($login_affiliate == 1?" and p.affiliate_id = '" . $login_id . "'":'') . " order by p.affiliate_payment_id DESC";
    } elseif (is_numeric($HTTP_GET_VARS['status'])) {
      $status = tep_db_prepare_input($HTTP_GET_VARS['status']);
      $payments_query_raw = "select p.* , s.affiliate_payment_status_name from " . TABLE_AFFILIATE_PAYMENT . " p , " . TABLE_AFFILIATE_PAYMENT_STATUS . " s where s.affiliate_payment_status_id = '" . tep_db_input($status) . "' and p.affiliate_payment_status = s.affiliate_payment_status_id and s.affiliate_language_id = '" . $languages_id . "' " . ($login_affiliate == 1?" and p.affiliate_id = '" . $login_id . "'":'') . " order by p.affiliate_payment_id DESC";
    } else {
      $payments_query_raw = "select p.* , s.affiliate_payment_status_name from " . TABLE_AFFILIATE_PAYMENT . " p , " . TABLE_AFFILIATE_PAYMENT_STATUS . " s where p.affiliate_payment_status = s.affiliate_payment_status_id and s.affiliate_language_id = '" . $languages_id . "' " . ($login_affiliate == 1?" and p.affiliate_id = '" . $login_id . "'":'') . " order by p.affiliate_payment_id DESC";
    }
...
?>
              ...
              ?>
              <tr>
                <td align=right colspan=6><?php echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_PAYMENT, 'pID=' . $pInfo->affiliate_payment_id. '&action=start_billing' ) . '">' . tep_image_button('button_affiliate_billing.gif', IMAGE_AFFILIATE_BILLING) . '</a>'; ?></td>
              </tr>
              <?php
              }
              ?>
