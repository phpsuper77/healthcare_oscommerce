<?php
/*
  $Id: currencies.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'save':
        ...
        $symbol_left = tep_db_prepare_input($HTTP_POST_VARS['symbol_left'], false);
        $symbol_right = tep_db_prepare_input($HTTP_POST_VARS['symbol_right'], false);
        ...
        if (tep_db_num_rows($check)){
          $messageStack->add(sprintf(TEXT_CURRENCY_CODE_ALREADY_EXISTS, $code), 'warning');
          if ($action == 'insert'){
            $action = 'new';
          }else{
            $action = 'edit';
