<?php
/*
  $Id: admin_members.php,v 1.1.1.1 2005/12/03 21:36:01 max Exp $
    ...
    switch ($HTTP_GET_VARS['action']) {
      ...
      case 'member_edit':
        ...
        if (in_array($HTTP_POST_VARS['admin_email_address'], $stored_email)) {
          ...
          tep_mail($HTTP_POST_VARS['admin_firstname'] . ' ' . $HTTP_POST_VARS['admin_lastname'], $HTTP_POST_VARS['admin_email_address'], ADMIN_EMAIL_EDIT_SUBJECT, sprintf(ADMIN_EMAIL_EDIT_TEXT, $HTTP_POST_VARS['admin_firstname'], tep_get_clickable_link(HTTP_SERVER . DIR_WS_ADMIN), $HTTP_POST_VARS['admin_email_address'], $hiddenPassword, STORE_OWNER), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
...
$header_title_menu_link=tep_href_link(FILENAME_ADMIN_MEMBERS, tep_get_all_get_params(array('selected_box')) . 'selected_box=administrator');
...
<?php
  ...
  switch ($HTTP_GET_VARS['action']) {  
    ...
    case 'define_group':      
      ...
      if ($HTTP_GET_VARS['gPath'] == 1) {
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS, 'gID=' . $HTTP_GET_VARS['gPath']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a><br>');      
      }
      break;
    case 'show_group': 
      $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_EDIT_GROUP . '</b>');
